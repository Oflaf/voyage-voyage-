...
const ignitionState = {
    rect: { x: 879, y: 769, width: 70, height: 70 }, // Pozycja i wymiary na desce rozdzielczej
    isTryingToStart: false,      // Czy gracz przytrzymuje kluczyk, próbując odpalić zgaszone auto
    wasManuallyShutDown: false   // Flaga, czy auto zostało już zgaszone stacyjką
};

const keyringState = {
    // Punkt zaczepienia dla obu breloczków, wyśrodkowany na stacyjce
    anchorX: ignitionState.rect.x + ignitionState.rect.width / 2,
    anchorY: ignitionState.rect.y + ignitionState.rect.height / 2, // Zmieniono na środek, aby lepiej wisiały

    // Tablica przechowująca indywidualny stan każdego breloczka
    keyrings: [
        {
            image: null, // Zostanie przypisane po załadowaniu
            size: { width: 100, height: 100 }, // Dopasuj do wymiarów keyring1.png
            angle: 0.1, // Lekki kąt startowy dla zróżnicowania
            velocity: 0,
            stiffness: 0.0078, // Nieco inna sztywność niż drugi
            damping: 0.975
        },
        {
            image: null, // Zostanie przypisane po załadowaniu
            size: { width: 100, height: 100 }, // Dopasuj do wymiarów keyring2.png
            angle: -0.1,
            velocity: 0,
            stiffness: 0.0072,
            damping: 0.968
...
const stanMuchy = {
    x: 0,
    y: 0,
    stan: 'chodzi', // Możliwe stany: 'chodzi', 'lata', 'stoi'
    kat: 0,         // Kąt obrotu muchy

    // Cel ruchu
    celX: 0,
    celY: 0,

    // Timer do zmiany stanu (chodzenie <-> latanie)
    timerZmianyStanu: 0,
    minCzasChodzenia: 8,
    maxCzasChodzenia: 20,
    minCzasLatania: 5,
    ...
    // --- NOWE WŁAŚCIWOŚCI DO RUCHU SINUSOIDALNEGO ---
    czasRuchu: 0, // Globalny timer do sterowania funkcjami sinusoidalnymi

    // Parametry dla chodzenia
    predkoscChodzenia: 45,
    amplitudaLukuChodzenia: 0.8, // Jak bardzo ma "skręcać" w radianach
    czestotliwoscLukuChodzenia: 3.0,
    wariacjaPredkosciChodzenia: 0.3, // 0.5 = prędkość waha się o +/- 50%
    
    // Parametry dla latania
    predkoscLatania: 580,
    amplitudaLukuLatania: 0.8, // Szersze, bardziej chaotyczne łuki
    czestotliwoscLukuLatania: 4.2,
    wariacjaPredkosciLatania: 0.7, // Mniejsze wahania prędkości, ale szybszy lot

    // Parametry do zatrzymywania się podczas chodzenia
    czasDoZatrzymania: 0,
    czasZatrzymania: 0,
    minCzasDoZatrzymania: 1,
    maxCzasDoZatrzymania: 4,
    minCzasPostoju: 0.5,
    maxCzasPostoju: 2.5
};
...
function rozpocznijGre() {
    ...
    // --- NOWY KOD: Logika dla wskaźnika generowania drogi ---
    const roadGenIndicator = document.getElementById('ingame-road-gen-indicator');
    if (roadGenIndicator) {
        // 1. Natychmiast pokaż wskaźnik
        roadGenIndicator.classList.add('widoczny');
        console.log("Gra rozpoczęta, pokazuję wskaźnik generowania drogi na 30 sekund.");

        // 2. Ustaw timer, który ukryje wskaźnik po 30 sekundach
        setTimeout(() => {
            console.log("Minęło 30 sekund, ukrywam wskaźnik generowania drogi.");
            roadGenIndicator.classList.remove('widoczny');
            
            // 3. (Opcjonalnie) Usuń element z DOM po zakończeniu animacji zanikania
            setTimeout(() => {
                roadGenIndicator.remove();
            }, 500); // Czas musi być zgodny z 'transition' w CSS

        }, 30000); // 30 000 milisekund = 30 sekund
    }
    // --- KONIEC NOWEGO KODU ---
    inicjalizujMuche();
...
const oilCheckAreaState = {
    // UWAGA: Dostosuj pozycję (x, y), aby kwadrat znajdował się w dobrym miejscu na silniku.
    x: 1120,
    y: 610,
    width: 160,
    height: 160
...
// Stan dla obiektu bagnetu, gdy jest trzymany przez gracza
const dipstickState = {
    isShown: false,      // Czy bagnet jest aktualnie widoczny i trzymany
    x: 0,
    y: 0,
    width: 85,         
    height: 590,       

    // Właściwości animacji TYLKO DLA PASKA
    barAlpha: 0.0,          // ZMIANA: Aktualna przezroczystość paska (0.0 do 1.0)
    animationSpeed: 9.5,    // Szybkość animacji fade-in/out paska
...
const brakeCapState = {
    // Bazowe właściwości
    x: 1771, // Przykładowa pozycja w bagażniku, dostosuj w razie potrzeby
    y: 589,
    width: 100, // Przykładowy rozmiar, dostosuj do grafiki brakecap.png
    height: 92,
    ...
    currentY: 350, // Musi być takie samo jak y na starcie
    currentAngle: 0,
    isVisible: true,
    
    // Parametry konfiguracyjne animacji (takie same jak dla oilcap)
    wobbleFrequency: 45,
    wobbleAmplitude: 0.15,
    maxOffsetY: -20,         // <-- ZMIANA ZGODNIE Z PROŚBĄ: Krótszy zakres ruchu (-20 zamiast -40)
    scrollSensitivity: 0.00031
};
...
const oilCapState = {
    ...
    scrollSensitivity: 0.00011  // Czułość scrolla (mniejsza wartość = wolniejsze odkręcanie)
};
...
const cigaretteBoxState = {
    rect: { x: 450, y: 680, width: 130, height: 50 }
};

const heldCigaretteState = {
    isHeld: false,
    x: 0,
    y: 0,
    width: 107,          // Dostosuj do wymiarów cigarette.png
    height: 71,        // Dostosuj do wymiarów cigarette.png

    // Fizyka bujania (skopiowana z heldBeerState, z drobnymi zmianami)
    angle: 0,
    angleVelocity: 0,
    stiffness: 0.005,    // Nieco sztywniejszy niż piwo
    damping: 0.82,     // Nieco inne tłumienie
    flickFactor: 0.00085,
    lastMouseX: 0,
};

const smokingAnimationState = {
    isActive: false,
    timer: 0,

    // Konfiguracja czasu trwania poszczególnych etapów animacji
    stage1_duration: 1.2,
    stage2_duration: 1.0,
    stage3_duration: 1.5,
    stage4_duration: 1.0,
    ...
    // Konfiguracja animacji wjazdu/zjazdu
    slideInDuration: 1.2,
    slideOutDuration: 0.6,
    onScreenOffsetY: 0,
    offScreenOffsetY: 1000,
    currentVerticalOffset: 1400,
    
    // --- NOWY KOD: Konfiguracja chybotania (skopiowana z drinkingAnimationState) ---
    swayTime: 0,
    swayConfig: {
        amplitude: 5.5, // Nieco mniejsza amplituda niż przy piciu
        frequency: 5.8
    },
    currentOffsetX: 0,
    currentOffsetY: 0,
    ...
    // Konfiguracja dymu
    smokeParticles: [],
    isSpawningSmoke: false,
    smokeSpawnTimer: 0,
    // --- ZMIANA: Gęstszy dym ---
    smokeSpawnInterval: 0.12 // Znacznie częściej tworzymy nową cząstkę
};

const drinkingAnimationState = {
    isActive: false,
    timer: 0,
    
    // Zaktualizowana konfiguracja czasu trwania dla 4 etapów
    stage1_a_duration: 0.3,     // Czas trwania drinkinga.png
    stage2_a2_duration: 0.4,    // Czas trwania drinkinga2.png
    stage3_a3_duration: 1.6,    // Czas trwania drinkinga3.png
    stage4_a2_duration: 0.1,    // Czas trwania finalnego drinkinga2.png
    totalDuration: 2.4,         // Suma powyższych

    currentImage: null,
    
    // Parametry drgania (bez zmian)
    swayTime: 0,
    swayConfig: {
        amplitude: 6.5,
        frequency: 6.1
    ...
    // --- NOWY KOD: Konfiguracja animacji wjazdu/zjazdu ---
    slideInDuration: 0.4,    // Czas trwania animacji wjazdu (w sekundach)
    slideOutDuration: 0.5,   // Czas trwania animacji zjazdu (w sekundach)

    // Definicja pozycji pionowej
    onScreenOffsetY: 50,     // Margines od dołu ekranu, gdy animacja jest na miejscu
    offScreenOffsetY: 600,   // Jak daleko pod ekranem animacja startuje/kończy (musi być większe niż wysokość obrazka)
    currentVerticalOffset: 600 // Aktualne przesunięcie pionowe, dynamicznie obliczane
};
...
const engineViewState = {
    ...
    smokeParticles: [],
    lastSmokeSpawn: 0,
    smokeSpawnInterval: 0.2, // Co ile sekund tworzyć nową cząstkę dymu
...
const trunkJugState = {
    // Pozycja i wymiary na ekranie (dostosuj, aby nie nachodził na inne przedmioty)
    x: 50,
    y: 74,
    width: 331,  // Te same wymiary co oilbase.png
    height: 320,
    ...
    // Właściwości animacji
    currentScale: 1.0,
    currentAngle: -0.2, // Lekkie przechylenie dla urozmaicenia
    isVisible: true,
    ...
    // Parametry animacji (identyczne jak dla oleju)
    swayTime: 0,
    swayFrequency: 8.0,
    swayAmplitude: 0.04,
    easeFactor: 0.1
...
// --- NOWY KOD: STAN DLA TRZYMANEGO DZBANKA (kopia heldOilState) ---
const heldJugState = {
    isHeld: false,
    x: 0,
    y: 0,
    width: 380,         // Dostosuj do wymiarów jug.png
    height: 540,        // Dostosuj do wymiarów jug.png
    
    // Fizyka bujania (identyczna jak dla oleju)
    angle: 0,
    angleVelocity: 0,
    stiffness: 0.004,
    damping: 0.78,
...
const radioState = {
    ...
    // --- NOWY KOD: Stan dla drgania radia ---
    shakeOffsetX: 0,          // Aktualne przesunięcie w osi X
    shakeOffsetY: 0,          // Aktualne przesunięcie w osi Y
    shakeTimer: 0,            // Czas trwania efektu (odlicza w dół)
    shakeDuration: 1.5,      // Jak długo ma trwać drganie (w sekundach)
    shakeIntensity: 2.8       // Jak mocno ma drgać (w pikselach)
};
...
function updateBrakeCap(deltaTime) {
    const state = brakeCapState; // Używamy nowego stanu
...
function drawBrakeCap() {
    const state = brakeCapState; // Używamy nowego stanu
    if (!state.isVisible || !assets.brakeCap || !assets.brakeCap.complete) { // Używamy nowej grafiki
        return;
    }

    ctx.save();
    // Ważne: W bagażniku nie ma drgania silnika (engineViewState.shakeOffsetX), więc go nie dodajemy
    ctx.translate(state.x + state.width / 2, state.currentY + state.height / 2);
    ctx.rotate(state.currentAngle);
    ctx.drawImage(assets.brakeCap, -state.width / 2, -state.height / 2, state.width, state.height); // Używamy nowej grafiki
    ctx.restore();
}
...
function drawOilCap() {
    ...
    // +++ DODAJ "engineViewState.shakeOffsetX" DO WSPÓŁRZĘDNEJ X +++
    ctx.translate(state.x + state.width / 2 + engineViewState.shakeOffsetX, state.currentY + state.height / 2);
    // Obracamy korek
    ctx.rotate(state.currentAngle);
...
const assets = {
  ...
  brakeCap: new Image(), // <-- DODAJ TĘ LINIĘ
    fly: new Image(),     // <--- DODAJ TĘ LINIĘ
  fly2: new Image(),    // <--- DODAJ TĘ LINIĘ
  trunk: new Image(),
    jugBase: new Image(), // Dzbanek w bagażniku
  jug: new Image(),     // Dzbanek trzymany przez gracza
  smokinga: new Image(),
  smokinga2: new Image(),
  oldcigarette: new Image(),
  oilBase: new Image(),
  cigarette: new Image(), // <--- DODAJ TĘ LINIĘ
  ...
  wheelBase: new Image(),
  wheel: new Image(),
dipstick: new Image(), // <--- DODAJ TĘ LINIĘ
...
fences: [],
  ...
  keyring2: new Image(),
    drinkinga:new Image(),
  drinkinga3: new Image(),
  drinkinga2: new Image(),
  blend: new Image(),   // <-- DODAJ TĘ LINIĘ
  blend2: new Image(),  // <-- DODAJ TĘ LINI
  washing: new Image(), // <-- DODAJ TĘ LINIĘ
   washing2: new Image(), // <-- DODAJ TĘ LINIĘ
  water: new Image(),
};
assets.keyring1.src = "img/keyring1.png"; menedzerZasobow.sledz(assets.keyring1);
assets.keyring2.src = "img/keyring2.png"; menedzerZasobow.sledz(assets.keyring2);

// Ten fragment jest kluczowy!
assets.keyring1.onload = () => { keyringState.keyrings[0].image = assets.keyring1; };
assets.keyring2.onload = () => { keyringState.keyrings[1].image = assets.keyring2; };
assets.fly.src = "img/fly.png"; menedzerZasobow.sledz(assets.fly);
assets.fly2.src = "img/fly2.png"; menedzerZasobow.sledz(assets.fly2);
assets.washing.src = "img/washing.png"; menedzerZasobow.sledz(assets.washing); // <-- DODAJ TĘ LINIĘ
assets.handleMirror.src = "img/handlemirror.png"; menedzerZasobow.sledz(assets.handleMirror);
assets.smokinga.src = "img/smokinga.png"; menedzerZasobow.sledz(assets.smokinga);
assets.smokinga2.src = "img/smokinga2.png"; menedzerZasobow.sledz(assets.smokinga2);
assets.washing2.src = "img/washing2.png"; menedzerZasobow.sledz(assets.washing2); // <-- DODAJ TĘ LINIĘ
assets.blend.src = "img/blend.png"; menedzerZasobow.sledz(assets.blend);     // <-- DODAJ TĘ LINIĘ
assets.blend2.src = "img/blend2.png"; menedzerZasobow.sledz(assets.blend2); // <-- DODAJ TĘ LINIĘ
assets.oldcigarette.src = "img/oldcigarette.png"; menedzerZasobow.sledz(assets.oldcigarette);
assets.jugBase.src = "img/jugbase.png"; menedzerZasobow.sledz(assets.jugBase);
assets.jug.src = "img/jug.png"; menedzerZasobow.sledz(assets.jug);
assets.cigarette.src = "img/cigarette.png"; menedzerZasobow.sledz(assets.cigarette);
...
assets.drinkinga2.src = "img/drinkinga2.png"; menedzerZasobow.sledz(assets.drinkinga2);
assets.drinkinga.src = "img/drinkinga.png"; menedzerZasobow.sledz(assets.drinkinga);
assets.brakeCap.src = "img/brakecap.png"; menedzerZasobow.sledz(assets.brakeCap); // <-- DODAJ TĘ LINIĘ
assets.drinkinga3.src = "img/drinkinga3.png"; menedzerZasobow.sledz(assets.drinkinga3);
...
assets.dipstick.src = "img/dipstick.png"; menedzerZasobow.sledz(assets.dipstick); // <--- DODAJ TĘ LINIĘ
assets.mud.src = "img/mud.png"; menedzerZasobow.sledz(assets.mud);
assets.mud2.src = "img/mud2.png"; menedzerZasobow.sledz(assets.mud2);
assets.road.src = "img/road.png"; menedzerZasobow.sledz(assets.road);
...
assets.trafficCone.src = "img/trafficcones.png"; menedzerZasobow.sledz(assets.trafficCone); // <--- NOWA LINIA
...
assets.backforest.src = "img/backforest.png"; menedzerZasobow.sledz(assets.backforest);
...
assets.blades.src = "img/blades.png"; menedzerZasobow.sledz(assets.blades);
assets.dust.src = "img/dust.png"; menedzerZasobow.sledz(assets.dust);
assets.dust_blue.src = "img/dust_blue.png"; menedzerZasobow.sledz(assets.dust_blue);
assets.hang.src = "img/hang.png"; menedzerZasobow.sledz(assets.hang);
assets.photo.src = "img/photo.png"; menedzerZasobow.sledz(assets.photo);
...
assets.water.src = "img/water.png"; menedzerZasobow.sledz(assets.water); // <-- NOWA LINIA

for (let i = 1; i <= 3; i++) {
    const img = new Image();
    // Zakładamy, że pliki nazywają się fence.png, fence2.png, fence3.png
    // Jeśli pierwszy plik nazywa się inaczej, trzeba by dodać warunek.
    const fileName = (i === 1) ? 'fence.png' : `fence${i}.png`;
    img.src = `img/${fileName}`;
    assets.fences.push(img);
    menedzerZasobow.sledz(img);
}
...
const oilDrops = []; 
const waterDrops = []; // <-- NOWA LINIA: Tablica na kropelki wody
const thrownBeers = []; // <--- DODAJ TĘ LINIĘ

const steamParticles = []; // <-- NOWY KOD: Tablica na cząsteczki pary (dust_blue.png)
const engineFryingState = { isPlaying: false }; // <-- NOWY KOD: Stan do zarządzania dźwiękiem smażenia

const thrownCigarettes = [];

const washingEffectState = {
    alpha: 0.0,           // Aktualna przezroczystość nakładki washing.png
    isActive: false,      // Czy efekt jest w trakcie (fade-in lub fade-out)
    isFadingIn: false,    // Faza: true = pojawianie się, false = znikanie
    maxAlpha: 0.85,       // Maksymalna widoczność efektu (0.0 do 1.0)
    fadeInSpeed: 2.5,     // Szybkość pojawiania się
    fadeOutSpeed: 0.2     // Szybkość znikania (wolniejsza dla lepszego efektu)
};

const splashEffectState = {
    alpha: 0.0,
    isActive: false,
    timer: 0,
    // Konfiguracja animacji
    fadeInDuration: 0.65,   // Jak szybko się pojawia
    holdDuration: 0.25,     // Jak długo jest w pełni widoczny
    fadeOutDuration: 2.6,  // Jak szybko znika
    maxAlpha: 0.86,
};

const spryskiwaczeState = {
    // --- NOWE WŁAŚCIWOŚCI DO STEROWANIA STRUMIENIEM ---
    isActive: false,      // Czy spryskiwacze są aktualnie włączone
    timer: 0,             // Timer odliczający czas trwania strumienia
    duration: 1.3,        // Jak długo (w sekundach) ma trwać strumień po naciśnięciu
    timerEmisji: 0,       // Wewnętrzny timer do kontrolowania częstotliwości "wystrzałów"
    ...
    konfiguracjaKropli: {
        // --- ZMIANA: To teraz wartości "na cykl", a nie na cały strzał ---
        interwalEmisji: 0.03,   // Co ile sekund generować nową porcję kropli
        liczbaNaEmisje: 10,      // Ile kropli generować w jednej porcji (z jednego punktu)

        maxCzasZycia: 1.3,
        minCzasZycia: 0.2,
        maxPredkoscPoczatkowaY: -10,
        minPredkoscPoczatkowaY: -2.5,
        poczatkowyRozrzutX: 1.1,
        silaRozpryskuV: 0.075,
        grawitacja: 0.04,
        minRozmiarPoczatkowy: 1,
        maxRozmiarPoczatkowy: 11,
        wspolczynnikWzrostu: 5.0
    }
};

// --- NOWY KOD: Stan do obsługi przytrzymania klawisza 'p' ---
const pKeyHoldState = {
    isDown: false,
    timer: 0,
    holdDuration: 0.5, // Czas w sekundach potrzebny do aktywacji spryskiwaczy
    actionTriggered: false
};

const sunVisorState = {
    // Tablica przechowująca stan każdej blendy z osobna
    visors: [
        // Blenda lewa
        {
            rect: { x: 390, y: 48, width: 628, height: 276 },
            isFlipped: true,
            isMirrored: false,

            // --- NOWE WŁAŚCIWOŚCI FIZYCZNE ---
            angle: 0,           // Aktualny kąt obrotu (w radianach)
            velocity: 0,        // Aktualna prędkość kątowa
            stiffness: 0.09,    // Sztywność "sprężyny" (jak szybko wraca do pozycji 0)
            damping: 0.85,      // Tłumienie (jak szybko wytraca prędkość i przestaje się bujać)
            clickForce: -0.12   // Siła "pchnięcia" przy kliknięciu (wartość ujemna = ruch w górę)
        },
        // Blenda prawa
        {
            rect: { x: 1160, y: 48, width: 598, height: 276 },
            isFlipped: true,
            isMirrored: true,
    ...
    // Ta właściwość pozostaje bez zmian
    yOffsetWhenFlipped: 80
};
...
// Konfiguracja spawnera much
const spawnerMuch = {
    timer: 10, // Pierwsza nowa mucha pojawi się po 10 sekundach
    minInterval: 15,
    maxInterval: 45,
    maxFlies: 12 // Maksymalna liczba much na ekranie
};

const mudState = {
    splatterAlpha: 0.0, // Przezroczystość dla mud.png (główne zabrudzenie)
    frameAlpha: 0.0,    // Przezroczystość dla mud2.png (krawędzie)
    ...
    // Konfiguracja
    maxAlpha: 0.98,
    dirtRate: 0.43,          // Szybkość brudzenia w trawie (BARDZO SZYBKO)
    cleanRateSplatter: 0.437,
    cleanRateFrame: 0.092,

    // --- NOWA WŁAŚCIWOŚĆ ---
    naturalDirtRate: 0.008   // Szybkość naturalnego brudzenia się szyby podczas jazdy (BARDZO WOLNO)
};
...
function updateSunVisors(deltaTime) {
    for (const visor of sunVisorState.visors) {
        if (visor.shakeTimer > 0) {
            visor.shakeTimer -= deltaTime;

            // Generuj losowe przesunięcie, gdy timer jest aktywny
            const intensity = visor.shakeIntensity;
            visor.currentShakeOffsetX = (Math.random() - 0.5) * intensity * 2;
            visor.currentShakeOffsetY = (Math.random() - 0.5) * intensity * 2;
        } else {
            // Zresetuj przesunięcie, gdy animacja się zakończy
            visor.currentShakeOffsetX = 0;
            visor.currentShakeOffsetY = 0;
        }
    }
}


function wybierzNowyCelDlaMuchy(mucha, czyNaSzybie) {
    if (czyNaSzybie) {
        mucha.celX = obszarSzyby.x + Math.random() * obszarSzyby.width;
        mucha.celY = obszarSzyby.y + Math.random() * obszarSzyby.height;
    } else {
        mucha.celX = Math.random() * canvas.width;
        mucha.celY = Math.random() * canvas.height;
    }
}

/**
 * Inicjalizuje system much na początku gry, tworząc pierwszą muchę.
 */
function inicjalizujMuche() {
    spawnNewFly();
}

/**
 * Aktualizuje logikę wszystkich much w każdej klatce.
 * @param {number} deltaTime - Czas od ostatniej klatki.
 */
function updateMuchy(deltaTime) {
    // Logika Spawnera
    spawnerMuch.timer -= deltaTime;
    if (spawnerMuch.timer <= 0 && muchy.length < spawnerMuch.maxFlies) {
        spawnNewFly();
        spawnerMuch.timer = spawnerMuch.minInterval + Math.random() * (spawnerMuch.maxInterval - spawnerMuch.minInterval);
        console.log(`Pojawiła się nowa mucha! Aktualna liczba: ${muchy.length}`);
    }

    // Pętla aktualizująca każdą muchę (od tyłu, bo będziemy usuwać elementy)
    for (let i = muchy.length - 1; i >= 0; i--) {
        const mucha = muchy[i];

        // --- NOWA LOGIKA DLA UDERZONEJ MUCHY ---
        if (mucha.stan === 'uderzona') {
            const grawitacja = 1.1;
            mucha.vy += grawitacja;
            mucha.y += mucha.vy * deltaTime * 10; // Mnożnik dla szybszego spadania
            mucha.kat += mucha.predkoscObrotu * deltaTime;

            // Usuń muchę, gdy spadnie poza ekran
            if (mucha.y > canvas.height + 50) {
                muchy.splice(i, 1);
            }
            continue; // Przejdź do następnej muchy, pomijając resztę logiki
        }
        
        // Reszta logiki ruchu (taka jak poprzednio, ale operuje na obiekcie 'mucha')
        mucha.timerZmianyStanu -= deltaTime;
        mucha.czasRuchu += deltaTime;

        if (mucha.timerZmianyStanu <= 0) {
            if (mucha.stan === 'chodzi' || mucha.stan === 'stoi') {
                mucha.stan = 'lata';
                mucha.timerZmianyStanu = mucha.minCzasLatania + Math.random() * (mucha.maxCzasLatania - mucha.minCzasLatania);
                wybierzNowyCelDlaMuchy(mucha, false);
                if (audio.unlocked && audio.flying && audio.flying.paused) {
                    audio.flying.volume = 0.57;
                    audio.flying.play().catch(e => console.warn("Nie udało się odtworzyć flying.mp3", e));
                }
            } else {
                mucha.stan = 'chodzi';
                mucha.timerZmianyStanu = mucha.minCzasChodzenia + Math.random() * (mucha.maxCzasChodzenia - mucha.minCzasChodzenia);
                wybierzNowyCelDlaMuchy(mucha, true);
                mucha.x = mucha.celX;
                mucha.y = mucha.celY;
                mucha.czasDoZatrzymania = mucha.minCzasDoZatrzymania + Math.random() * (mucha.maxCzasDoZatrzymania - mucha.minCzasDoZatrzymania);
                if (audio.unlocked && audio.flying && !audio.flying.paused) {
                    audio.flying.pause();
                }
            }
        ...
        if (mucha.stan === 'stoi') {
            mucha.czasZatrzymania -= deltaTime;
            if (mucha.czasZatrzymania <= 0) {
                mucha.stan = 'chodzi';
                mucha.czasDoZatrzymania = mucha.minCzasDoZatrzymania + Math.random() * (mucha.maxCzasDoZatrzymania - mucha.minCzasDoZatrzymania);
                wybierzNowyCelDlaMuchy(mucha, true);
            }
        ...
        const dx = mucha.celX - mucha.x;
        const dy = mucha.celY - mucha.y;
        const dystans = Math.sqrt(dx * dx + dy * dy);
        
        let bazowaPredkosc, amplitudaLuku, czestotliwoscLuku, wariacjaPredkosci, progOsiagnieciaCelu, czyNaSzybie;

        if (mucha.stan === 'chodzi') {
            mucha.czasDoZatrzymania -= deltaTime;
            if (mucha.czasDoZatrzymania <= 0) {
                mucha.stan = 'stoi';
                mucha.czasZatrzymania = mucha.minCzasPostoju + Math.random() * (mucha.maxCzasPostoju - mucha.minCzasPostoju);
                continue;
            }
            bazowaPredkosc = mucha.predkoscChodzenia;
            amplitudaLuku = mucha.amplitudaLukuChodzenia;
            czestotliwoscLuku = mucha.czestotliwoscLukuChodzenia;
            wariacjaPredkosci = mucha.wariacjaPredkosciChodzenia;
            progOsiagnieciaCelu = 5;
            czyNaSzybie = true;
        } else { // lata
            bazowaPredkosc = mucha.predkoscLatania;
            amplitudaLuku = mucha.amplitudaLukuLatania;
            czestotliwoscLuku = mucha.czestotliwoscLukuLatania;
            wariacjaPredkosci = mucha.wariacjaPredkosciLatania;
            progOsiagnieciaCelu = 20;
            czyNaSzybie = false;
        }

        if (dystans < progOsiagnieciaCelu) {
            wybierzNowyCelDlaMuchy(mucha, czyNaSzybie);
            if (mucha.stan === 'chodzi') {
                mucha.czasDoZatrzymania = mucha.minCzasDoZatrzymania + Math.random() * (mucha.maxCzasDoZatrzymania - mucha.minCzasDoZatrzymania);
            }
        }
        
        const katDoCelu = Math.atan2(dy, dx);
        const katLuku = Math.sin(mucha.czasRuchu * czestotliwoscLuku) * amplitudaLuku;
        mucha.kat = katDoCelu + katLuku;
        const modyfikatorPredkosci = 1 + (Math.sin(mucha.czasRuchu * czestotliwoscLuku * 1.5) * wariacjaPredkosci);
        const aktualnaPredkosc = bazowaPredkosc * modyfikatorPredkosci;
        mucha.x += Math.cos(mucha.kat) * aktualnaPredkosc * deltaTime;
        mucha.y += Math.sin(mucha.kat) * aktualnaPredkosc * deltaTime;
    }
}

function drawMuchy() {
    // Pętla rysująca każdą muchę
    for (const mucha of muchy) {
        let obrazekMuchy;
        if (mucha.stan === 'chodzi' || mucha.stan === 'stoi' || mucha.stan === 'uderzona') {
            obrazekMuchy = assets.fly;
        } else {
            obrazekMuchy = assets.fly2;
        }

        if (!obrazekMuchy || !obrazekMuchy.complete) continue;

        ctx.save();
        ctx.translate(mucha.x, mucha.y);
        ctx.rotate(mucha.kat);
        ...
        ctx.drawImage(obrazekMuchy, -rozmiar / 2, -rozmiar / 2, rozmiar, rozmiar);
        ctx.restore();
    }
}
/**
 * Rysuje obie blendy przeciwsłoneczne na ekranie.
 */
function drawSunVisors() {
    // Nie rysuj, jeśli grafiki nie są załadowane
    if (!assets.blend.complete || !assets.blend2.complete) {
        return;
    }

    for (const visor of sunVisorState.visors) {
        // === TUTAJ JEST ZMIANA! ===
        // Odwracamy logikę: Domyślnie (isFlipped: false) jest blend2, po kliknięciu (isFlipped: true) jest blend.
        const imageToDraw = visor.isFlipped ? assets.blend : assets.blend2;
        
        // Oblicz pozycję Y z uwzględnieniem przesunięcia w górę dla opuszczonej blendy
        let y = visor.rect.y;
        // Ta logika pozostaje bez zmian - opuszczona blenda (isFlipped: true) jest wyżej
        if (visor.isFlipped) {
            y -= sunVisorState.yOffsetWhenFlipped;
        }

        // Dodaj efekt trzęsienia do finalnych koordynatów
        const drawX = visor.rect.x + visor.currentShakeOffsetX;
        const drawY = y + visor.currentShakeOffsetY;

        ctx.save();
        
        // Obsługa lustrzanego odbicia dla prawej blendy
        if (visor.isMirrored) {
            // Przesuń punkt odniesienia do prawego górnego rogu blendy
            ctx.translate(drawX + visor.rect.width, drawY);
            // Odwróć canvas horyzontalnie
            ctx.scale(-1, 1);
            // Narysuj obrazek od nowego punktu (0,0)
            ctx.drawImage(imageToDraw, 0, 0, visor.rect.width, visor.rect.height);
        } else {
            // Narysuj normalnie
            ctx.drawImage(imageToDraw, drawX, drawY, visor.rect.width, visor.rect.height);
        }
...
function updateMudEffect(deltaTime) {
    // 1. Logika brudzenia szyby (pozostaje bez zmian)
    if (gameState.isInGrass && gameState.speedKmH > 5) {
        mudState.splatterAlpha += mudState.dirtRate * deltaTime;
        mudState.frameAlpha += mudState.dirtRate * deltaTime;
    }
    if (gameState.speedKmH > 10 && !gameState.isInGrass) {
        mudState.splatterAlpha += mudState.naturalDirtRate * deltaTime;
        mudState.frameAlpha += mudState.naturalDirtRate * deltaTime;
    }
    mudState.splatterAlpha = Math.min(mudState.splatterAlpha, mudState.maxAlpha);
    mudState.frameAlpha = Math.min(mudState.frameAlpha, mudState.maxAlpha);

    // 2. Nowa, rozbudowana logika czyszczenia szyby
    if (wipersState.active) {
        let cleaningMultiplier; // Mnożnik prędkości czyszczenia

        // Sprawdzamy, czy spryskiwacze są włączone, aby ustawić odpowiedni mnożnik
        if (spryskiwaczeState.isActive) {
            // Wycieraczki + Płyn = czyszczenie 2x szybsze
            cleaningMultiplier = 2.5;
        } else {
            // Tylko wycieraczki (na sucho) = czyszczenie bardzo wolne
            cleaningMultiplier = 0.25; // Używamy tylko 5% bazowej prędkości. Możesz dostosować tę wartość!
        }

        // Aplikujemy czyszczenie z odpowiednią siłą
        mudState.splatterAlpha -= mudState.cleanRateSplatter * cleaningMultiplier * deltaTime;
        mudState.frameAlpha -= mudState.cleanRateFrame * cleaningMultiplier * deltaTime;

        // Upewniamy się, że alpha nie spadnie poniżej zera
        mudState.splatterAlpha = Math.max(0, mudState.splatterAlpha);
        mudState.frameAlpha = Math.max(0, mudState.frameAlpha);
    }
}

/**
 * Rysuje warstwy błota na szybie.
 */
function drawMud() {
    // Sprawdzamy, czy w ogóle jest coś do narysowania, aby oszczędzić zasoby
    if (mudState.splatterAlpha <= 0 && mudState.frameAlpha <= 0) {
        return;
    }

    // Pobieramy pozycję i wymiary z obiektu stanu
    const { x, y, width, height } = mudState.position;

    // Rysowanie głównego zabrudzenia (mud.png)
    if (mudState.splatterAlpha > 0 && assets.mud && assets.mud.complete) {
        ctx.globalAlpha = mudState.splatterAlpha;
        ctx.drawImage(assets.mud, x, y, width, height);
    }

    // Rysowanie zabrudzenia na krawędziach (mud2.png)
    if (mudState.frameAlpha > 0 && assets.mud2 && assets.mud2.complete) {
        ctx.globalAlpha = mudState.frameAlpha;
        ctx.drawImage(assets.mud2, x, y, width, height);
    }
}

function updateTrunkJug(deltaTime) {
    const state = trunkJugState;
    if (state.isHovered) {
...
function drawTrunkJug() {
    if (!trunkJugState.isVisible || !assets.jugBase || !assets.jugBase.complete) {
        return;
    }
    const state = trunkJugState;
    ...
    ctx.rotate(state.currentAngle);
    ctx.drawImage(assets.jugBase, -finalWidth / 2, -finalHeight / 2, finalWidth, finalHeight);
    ctx.restore();
}

// --- NOWY KOD: Funkcje dla trzymanego dzbanka (kopia funkcji dla oleju) ---

function updateHeldJug(deltaTime) {
    if (!heldJugState.isHeld) return;
    const restoringForce = -heldJugState.angle * heldJugState.stiffness;
    heldJugState.angleVelocity += restoringForce;
    heldJugState.angleVelocity *= heldJugState.damping;
    heldJugState.angle += heldJugState.angleVelocity;
}

function drawHeldJug() {
    if (!heldJugState.isHeld || !assets.jug || !assets.jug.complete) {
        return;
    }
    ctx.save();
    ctx.translate(heldJugState.x, heldJugState.y);
    ctx.rotate(heldJugState.angle);
    ctx.drawImage(
        assets.jug,
        -heldJugState.width / 2, 
        -heldJugState.height / 2, 
        heldJugState.width, 
        heldJugState.height
    );
    ctx.restore();
...
// --- NOWY KOD: Funkcje dla kropelek wody (kopia funkcji dla oleju Z MODYFIKACJĄ) ---

function updateWaterDrops(deltaTime) {
    const gravity = 0.25; 
    ...
    for (let i = waterDrops.length - 1; i >= 0; i--) {
        ...
        // !!! WAŻNA MODYFIKACJA !!!
        // USUNIĘTO BLOK SPRAWDZAJĄCY WLEWANIE DO SILNIKA.
        // Woda po prostu znika po czasie, nigdzie nie jest wlewana.
...
function drawWaterDrops() {
    if (waterDrops.length === 0 || !assets.water || !assets.water.complete) {
        return;
    }
    ctx.save();
...
function updateEngineView(deltaTime) {
    ...
    if (gameState.isOverheating) {
        ...
        engineViewState.engineAlpha = 1.0 - easedProgress;

        // 3. Generowanie dymu
        engineViewState.lastSmokeSpawn += deltaTime;
        
        // --- ZMIANA 1: ZWIĘKSZENIE ILOŚCI DYMU ---
        // Zmniejszamy wartość `smokeSpawnInterval`, aby dym pojawiał się częściej.
        // Jeśli miałeś tu `*3`, usuń to, aby dym generował się znacznie szybciej.
        if (engineViewState.lastSmokeSpawn > engineViewState.smokeSpawnInterval) { 
            engineViewState.lastSmokeSpawn = 0;
            
            engineViewState.smokeParticles.push({
                x: canvas.width / 2 + (Math.random() - 0.5) * 300,
                y: canvas.height / 2 + Math.random() * 350,
                size: 30 + Math.random() * 280,
                
                // --- ZMIANA 2: ZWIĘKSZENIE WIDOCZNOŚCI (ALPHA) ---
                // Stare wartości: alpha: 0.01 + Math.random() * 0.11,
                // Nowe wartości dają znacznie gęstszy, bardziej widoczny dym.
                alpha: 0.2 + Math.random() * 0.6, // Zakres od 0.3 do 0.7

                vy: -0.2 - Math.random() * 0.73,
                growth: 1.2 + Math.random() * 9.5,
                lifetime: 0,
                maxLifetime: 6.0 + Math.random() * 9.0
            });
        }
    } else {
        // --- LOGIKA DLA PRACUJĄCEGO SILNIKA (bez zmian) ---
    ...
    // 4. Aktualizacja wszystkich cząsteczek dymu (bez zmian)
    for (let i = engineViewState.smokeParticles.length - 1; i >= 0; i--) {
        ...
        if (p.lifetime >= p.maxLifetime) {
            ...
            // Tutaj alpha jest dynamicznie zmniejszana, ale bazowa wartość z góry ma znaczenie
            p.alpha = (0.2 + Math.random() * 0.4) * (1 - lifeProgress);
        }
    }
}
...
function drawDrinkingAnimation() {
    if (!drinkingAnimationState.isActive || !drinkingAnimationState.currentImage || !drinkingAnimationState.currentImage.complete) {
        return;
    }
    const state = drinkingAnimationState;
    const img = state.currentImage;
    const scale = 1.2;
    const width = img.naturalWidth * scale;
    const height = img.naturalHeight * scale;

    // Pozycja X uwzględnia tylko drganie na boki
    const drawX = (canvas.width / 2) - (width / 2) + state.currentOffsetX;
    
    // --- ZMODYFIKOWANA POZYCJA Y ---
    // Bazowa pozycja to dół ekranu minus wysokość obrazka.
    // Następnie dodajemy drganie góra-dół (currentOffsetY)
    // oraz główne przesunięcie od animacji wjazdu/zjazdu (currentVerticalOffset).
    const drawY = (canvas.height - height) + state.currentOffsetY + state.currentVerticalOffset;

    ctx.drawImage(img, drawX, drawY, width, height);
}

/**
 * Aktualizuje pozycję i rotację wszystkich rzuconych butelek.
 */
function updateThrownBeers(deltaTime) {
    for (let i = thrownBeers.length - 1; i >= 0; i--) {
        const beer = thrownBeers[i];

        // Prosta fizyka
        beer.vy += beer.gravity; // Grawitacja
        beer.x += beer.vx;
        beer.y += beer.vy;
        beer.angle += beer.rotationSpeed;

        // Usuń butelkę, gdy wyleci daleko poza ekran
        if (beer.y > canvas.height + 200) {
...
/**
 * Rysuje wszystkie aktywne, rzucone butelki.
 */
function drawThrownBeers() {
    if (thrownBeers.length === 0 || !assets.beer.complete) return;

    for (const beer of thrownBeers) {
        ctx.save();
        ctx.translate(beer.x, beer.y);
        ctx.rotate(beer.angle);
        ctx.drawImage(
            assets.beer,
            -heldBeerState.width / 2,
            -heldBeerState.height / 2,
...
function updateDrinkingAnimation(deltaTime) {
    if (!drinkingAnimationState.isActive) return;

    const state = drinkingAnimationState;
    state.timer += deltaTime;
    state.swayTime += deltaTime;

    // --- NOWA LOGIKA: Obliczanie pozycji pionowej (animacja wjazdu/zjazdu) ---
    
    // Funkcja pomocnicza dla płynnego ruchu (ease-in-out)
    const easeInOut = t => 0.5 - 0.5 * Math.cos(t * Math.PI);

    // Faza 1: Wjazd na ekran
    if (state.timer < state.slideInDuration) {
        const progress = state.timer / state.slideInDuration;
        const easedProgress = easeInOut(progress);
        // Płynne przejście z pozycji pod ekranem do pozycji na ekranie
        state.currentVerticalOffset = state.offScreenOffsetY - (state.offScreenOffsetY - state.onScreenOffsetY) * easedProgress;
    }
    // Faza 3: Zjazd pod ekran (rozpoczyna się przed końcem całej animacji)
    else if (state.timer > state.totalDuration - state.slideOutDuration) {
        const timeIntoSlideOut = state.timer - (state.totalDuration - state.slideOutDuration);
        const progress = timeIntoSlideOut / state.slideOutDuration;
        const easedProgress = easeInOut(progress);
        // Płynne przejście z pozycji na ekranie do pozycji pod ekranem
        state.currentVerticalOffset = state.onScreenOffsetY + (state.offScreenOffsetY - state.onScreenOffsetY) * easedProgress;
    }
    // Faza 2: Animacja jest na swoim miejscu na ekranie
    else {
        state.currentVerticalOffset = state.onScreenOffsetY;
    }
    
    // --- Logika zmiany obrazków (przeniesiona, bez zmian merytorycznych) ---
    const timepoint1 = state.stage1_a_duration;
    const timepoint2 = timepoint1 + state.stage2_a2_duration;
    const timepoint3 = timepoint2 + state.stage3_a3_duration;

    if (state.timer < timepoint1) {
        state.currentImage = assets.drinkinga;
    } else if (state.timer < timepoint2) {
        state.currentImage = assets.drinkinga2;
    } else if (state.timer < timepoint3) {
        state.currentImage = assets.drinkinga3;
    } else {
        state.currentImage = assets.drinkinga2;
    }

    // Obliczanie naturalnego drgania (bez zmian)
    state.currentOffsetX = Math.sin(state.swayTime * state.swayConfig.frequency) * state.swayConfig.amplitude;
    state.currentOffsetY = Math.cos(state.swayTime * state.swayConfig.frequency * 0.7) * state.swayConfig.amplitude * 0.5;

    // Zakończenie animacji i rzucenie butelki (bez zmian)
    if (state.timer >= state.totalDuration) {
        ...
        thrownBeers.push(newThrownBeer);
    }
}

function uruchomSpryskiwacze() {
    // Jeśli strumień jest już aktywny, nie rób nic
    if (spryskiwaczeState.isActive) return;

    // Odtwórz dźwięk (jednorazowo na starcie strumienia)
    if (audio.unlocked && audio.washer) {
        audio.washer.currentTime = 0;
        audio.washer.volume = 0.8;
        audio.washer.play().catch(e => console.warn("Nie udało się odtworzyć washer.mp3", e));
    }

    // Aktywuj strumień i zresetuj timery
    spryskiwaczeState.isActive = true;
    spryskiwaczeState.timer = 0;
    spryskiwaczeState.timerEmisji = 0;
}
/**
 * Aktualizuje pozycję, rozmiar i przezroczystość każdej kropli wody.
 */

function updateSplashEffect(deltaTime) {
    const state = splashEffectState;
    if (!state.isActive) return;

    state.timer += deltaTime;
    const totalDuration = state.fadeInDuration + state.holdDuration + state.fadeOutDuration;

    if (state.timer < state.fadeInDuration) {
        // Faza 1: Pojawianie się
        const progress = state.timer / state.fadeInDuration;
        state.alpha = progress * state.maxAlpha;
    } else if (state.timer < state.fadeInDuration + state.holdDuration) {
        // Faza 2: Utrzymanie
        state.alpha = state.maxAlpha;
    } else if (state.timer < totalDuration) {
        // Faza 3: Znikane
        const timeIntoFadeOut = state.timer - (state.fadeInDuration + state.holdDuration);
        const progress = timeIntoFadeOut / state.fadeOutDuration;
        state.alpha = state.maxAlpha * (1 - progress);
    } else {
        // Koniec animacji
        state.alpha = 0;
        state.isActive = false;
...
// --- NOWY KOD: Funkcja rysująca efekt "plusku" na szybie ---
function drawSplashEffect() {
    if (splashEffectState.alpha <= 0 || !assets.washing2 || !assets.washing2.complete) {
        return;
    }
    const { x, y, width, height } = mudState.position; // Używamy tych samych koordynatów
    ctx.save();
    ctx.globalAlpha = splashEffectState.alpha;
    ctx.drawImage(assets.washing2, x, y, width, height);
    ctx.restore();
}

function updateSpryskiwacze(deltaTime) {
    const state = spryskiwaczeState;
    const konf = state.konfiguracjaKropli;

    // --- CZĘŚĆ 1: ZARZĄDZANIE STRUMIENIEM I DŹWIĘKIEM ---
    if (state.isActive) {
        state.timer += deltaTime;
        state.timerEmisji += deltaTime;

        if (audio.unlocked && audio.washer && audio.washer.paused) {
            audio.washer.currentTime = 0;
            audio.washer.play().catch(e => console.warn("Nie udało się odtworzyć washer.mp3", e));
        }

        // --- ZMIANA: Usunięto warunek '!wipersState.active' ---
        // Teraz efekt "plusku" uruchomi się zawsze, gdy spryskiwacz jest aktywny,
        // o ile animacja plusku nie jest już w toku.
        if (!splashEffectState.isActive) {
            splashEffectState.isActive = true;
            splashEffectState.timer = 0;
        }
        ...
        if (state.timerEmisji >= konf.interwalEmisji) {
            state.timerEmisji -= konf.interwalEmisji;

            for (const punkt of state.punktyStartowe) {
                for (let i = 0; i < konf.liczbaNaEmisje; i++) {
                    const katRozprysku = (Math.random() * 2) - 1;
                    const nowaKropla = {
                        x: punkt.x,
                        y: punkt.y,
                        vx: katRozprysku * konf.poczatkowyRozrzutX,
                        vy: konf.minPredkoscPoczatkowaY + Math.random() * (konf.maxPredkoscPoczatkowaY - konf.minPredkoscPoczatkowaY),
                        grawitacja: konf.grawitacja,
                        rozmiar: konf.minRozmiarPoczatkowy + Math.random() * (konf.maxRozmiarPoczatkowy - konf.minRozmiarPoczatkowy),
                        alpha: 1.0,
                        czasZycia: 0,
                        maxCzasZycia: konf.minCzasZycia + Math.random() * (konf.maxCzasZycia - konf.minCzasZycia),
                        katRozprysku: katRozprysku
                    };
                    state.krople.push(nowaKropla);
                }
            }
        }
    } else {
        if (audio.washer && !audio.washer.paused) {
            audio.washer.pause();
        }
    }

    // --- CZĘŚĆ 2: AKTUALIZACJA WSZYSTKICH ISTNIEJĄCYCH KROPLI (zawsze) ---
    if (state.krople.length === 0) return;

    for (let i = state.krople.length - 1; i >= 0; i--) {
        const kropla = state.krople[i];
        kropla.czasZycia += deltaTime;

        if (kropla.czasZycia >= kropla.maxCzasZycia || (wipersState.active && kropla.czasZycia >= 0.5)) {
            state.krople.splice(i, 1);
            continue;
        }
        
        kropla.vx += kropla.katRozprysku * konf.silaRozpryskuV;
        kropla.vy += kropla.grawitacja;
        kropla.x += kropla.vx;
        kropla.y += kropla.vy;
        kropla.rozmiar += konf.wspolczynnikWzrostu * deltaTime;
        kropla.alpha = 1.0 - (kropla.czasZycia / kropla.maxCzasZycia);
    }
}


/**
 * Rysuje wszystkie aktywne krople wody na ekranie.
 */
function drawSpryskiwacze() {
    if (spryskiwaczeState.krople.length === 0 || !assets.water || !assets.water.complete) {
        return;
    }

    ctx.save();
    for (const kropla of spryskiwaczeState.krople) {
        ctx.globalAlpha = kropla.alpha;
        ctx.drawImage(
            assets.water,
            kropla.x - kropla.rozmiar / 2,
            kropla.y - kropla.rozmiar / 2,
            kropla.rozmiar,
            kropla.rozmiar
        );
    }
...
function updateViewTransition(deltaTime) {
    ...
    // Faza 1: Ściemnianie ekranu (fade-out)
        ...
        if (state.progress >= 1.0) {
            ...
            // --- NOWY KOD ---
            // Resetujemy licznik kropli za każdym razem, gdy przechodzimy do widoku silnika.
            if (currentView === 'engine') {
                engineViewState.coolingDropsCount = 0; 
                console.log("Licznik kropli do chłodzenia zresetowany.");
            }
...
const gameState = {
  ...
  coneHitShake: { active: false, timer: 0, duration: 0.25, intensity: 8.5 },
  ignitionShake: { active: false, intensity: 4.5,
      currentIntensity: 0.0, // Aktualna moc drgań (będzie się zmieniać płynnie)
      maxIntensity: 4.5,     // Maksymalna moc drgań podczas próby odpalenia
      easeInFactor: 0.007,     // Szybkość narastania drgań (mniejsza = wolniej)
      easeOutFactor: 0.04    // Szybkość wygasania drgań (mniejsza = wolniej)
  },
...
function createMapButton() { const button = document.createElement('button'); button.id = 'mapButton'; button.style.position = 'absolute'; button.style.zIndex = '1000'; button.style.backgroundColor = "rgba(65, 65, 65, 0)"; button.style.border = 'none'; button.style.cursor = ''; button.addEventListener('click', toggleMap); document.body.appendChild(button); }
function createPhotoButton() { const button = document.createElement('button'); button.id = 'photoButton'; button.style.position = 'absolute'; button.style.zIndex = '1000'; button.style.backgroundColor = "rgba(65, 65, 65, 0)"; button.style.border = 'none'; button.style.cursor = ''; button.addEventListener('click', togglePhoto); document.body.appendChild(button); }
...
function updateDipstick(deltaTime) {
    // 1. Logika animacji przezroczystości DLA PASKA
    const targetAlpha = dipstickState.isShown ? 1.0 : 0.0;
    
    // Animujemy przezroczystość paska, dążąc do wartości docelowej
    if (Math.abs(dipstickState.barAlpha - targetAlpha) > 0.001) {
        const direction = targetAlpha > dipstickState.barAlpha ? 1 : -1;
        dipstickState.barAlpha += direction * dipstickState.animationSpeed * deltaTime;
        dipstickState.barAlpha = Math.max(0.0, Math.min(1.0, dipstickState.barAlpha));
    }

    // 2. Logika fizyki DLA BAGNETU (działa tylko, gdy jest widoczny)
    // ZMIANA: Jeśli bagnet nie jest pokazywany, resetujemy fizykę i kończymy
    if (!dipstickState.isShown) {
        dipstickState.angle = 0;
        dipstickState.angleVelocity = 0;
        return; 
    }

    // Standardowa fizyka sprężynowania
    const restoringForce = -dipstickState.angle * dipstickState.stiffness;
    dipstickState.angleVelocity += restoringForce;
    dipstickState.angleVelocity *= dipstickState.damping;
    dipstickState.angle += dipstickState.angleVelocity;
}

/**
 * Rysuje bagnet przy kursorze myszy.
 */

function drawIgnitionKey() {
    // Rysuj tylko w kokpicie
    if (currentView !== 'cockpit') {
        return;
    }
    const { x, y, width, height } = ignitionState.rect;
    ctx.fillRect(x, y, width, height);
}

function drawDipstickOilBar() {
    // ZMIANA: Warunek zależy teraz od przezroczystości paska
    if (dipstickState.barAlpha <= 0) {
        return;
    }

    const state = dipstickState;
    ctx.save();
    
    // Zastosuj te same transformacje co bagnet, aby pasek poruszał się razem z nim
    // ZMIANA: Używamy barAlpha do sterowania przezroczystością
    ctx.globalAlpha = state.barAlpha;
    ctx.translate(state.x, state.y);
    ctx.rotate(state.angle);

    // Definicje paska (bez zmian)
    const barWidth = 11;
    const barHeight = 140;
    ...
    // Gradient
    const gradient = ctx.createLinearGradient(offsetX, offsetY + barHeight, offsetX, offsetY);
    gradient.addColorStop(0, "red");
    gradient.addColorStop(0.5, "yellow");
    gradient.addColorStop(1, "lime");

    // Wypełnienie
    const fillHeight = (gameState.oilAmount / 100) * barHeight;
    ctx.fillStyle = gradient;
    ctx.fillRect(offsetX, offsetY + (barHeight - fillHeight), barWidth, fillHeight);
...
function drawDipstick() {
    // ZMIANA: Rysuj tylko, jeśli flaga isShown jest prawdziwa (natychmiastowy efekt)
    if (!dipstickState.isShown || !assets.dipstick || !assets.dipstick.complete) {
        return;
    }
    ...
    // USUNIĘTO: ctx.globalAlpha = state.alpha; // To sprawia, że bagnet jest zawsze w pełni widoczny
...
/**
 * Rysuje interfejs paska poziomu oleju poniżej kursora, gdy bagnet jest wyciągnięty.
 */
...
function handleRadioVolumeScroll(e) {
    ...
    // MODYFIKACJA: Sprawdzamy, czy głośność faktycznie się zmieniła
    if (oldVolume !== radioState.volume) {
        // DODAJ TĘ LINIĘ: Uruchomienie efektu drgania
        radioState.shakeTimer = radioState.shakeDuration;
...
function updateConeHits(deltaTime) {
    for (let i = activeConeHits.length - 1; i >= 0; i--) {
        const cone = activeConeHits[i];
        cone.lifetime += deltaTime;
        
        if (cone.lifetime >= cone.maxLifetime) {
            activeConeHits.splice(i, 1); // Usuń pachołek po zakończeniu animacji
            continue;
        }
        
        // Aktualizacja animacji
        cone.rotation += cone.rotationSpeed * deltaTime;
        cone.y -= cone.velocityY * deltaTime; // Pachołek leci "w górę" ekranu
        cone.alpha = 1.0 - (cone.lifetime / cone.maxLifetime); // Efekt zanikania
    }
}

/**
 * Rysuje wszystkie aktywne, uderzone pachołki.
 */
function drawConeHits() {
    if (!assets.trafficCone.complete || activeConeHits.length === 0) {
        return;
    }

    ctx.save();
    for (const cone of activeConeHits) {
        ctx.globalAlpha = cone.alpha;
        ctx.save();
        
        // Przesuń, obróć i narysuj
        ...
        ctx.drawImage(
            ...
            -cone.size / 2, 
            cone.size, 
            cone.size * (assets.trafficCone.naturalHeight / assets.trafficCone.naturalWidth)
        );
...
function updateSmokingAnimation(deltaTime) {
    if (!smokingAnimationState.isActive) {
        // --- NOWA LOGIKA: Aktualizuj istniejący dym nawet po zakończeniu animacji ---
        // Ta pętla musi tu być, aby dym mógł naturalnie zaniknąć.
        const state = smokingAnimationState;
        for (let i = state.smokeParticles.length - 1; i >= 0; i--) {
            ...
            if (p.lifetime >= p.maxLifetime) {
                ...
                // Używamy potęgi, aby dym zanikał wolniej na początku i szybciej na końcu
                const lifeProgress = p.lifetime / p.maxLifetime;
                p.alpha = p.baseAlpha * (1 - Math.pow(lifeProgress, 2));
            }
        }
        return; // Zakończ, jeśli animacja ręki jest nieaktywna
    }

    const state = smokingAnimationState;
    state.timer += deltaTime;
    
    // Obliczanie chybotania
    state.swayTime += deltaTime;
    state.currentOffsetX = Math.sin(state.swayTime * state.swayConfig.frequency) * state.swayConfig.amplitude;
    state.currentOffsetY = Math.cos(state.swayTime * state.swayConfig.frequency * 0.7) * state.swayConfig.amplitude * 0.5;
    
    // Animacja wjazdu/zjazdu
    const easeInOut = t => 0.5 - 0.5 * Math.cos(t * Math.PI);
    if (state.timer < state.slideInDuration) {
        const progress = state.timer / state.slideInDuration;
        state.currentVerticalOffset = state.offScreenOffsetY - (state.offScreenOffsetY - state.onScreenOffsetY) * easeInOut(progress);
    } else if (state.timer > state.totalDuration - state.slideOutDuration) {
        const timeIntoSlideOut = state.timer - (state.totalDuration - state.slideOutDuration);
        const progress = timeIntoSlideOut / state.slideOutDuration;
        state.currentVerticalOffset = state.onScreenOffsetY + (state.offScreenOffsetY - state.onScreenOffsetY) * easeInOut(progress);
    } else {
        state.currentVerticalOffset = state.onScreenOffsetY;
    }

    // Logika zmiany obrazków i dymu
    const timepoint1 = state.stage1_duration;
    const timepoint2 = timepoint1 + state.stage2_duration;
    const timepoint3 = timepoint2 + state.stage3_duration;
    const timepoint4 = timepoint3 + state.stage4_duration;

    state.isSpawningSmoke = false;

    if (state.timer < timepoint1) {
        state.currentImage = assets.smokinga;
    } else if (state.timer < timepoint2) {
        state.currentImage = assets.smokinga2;
        state.currentVerticalOffset += 20; 
    } else if (state.timer < timepoint3) {
        state.currentImage = assets.smokinga;
        state.isSpawningSmoke = true;
    } else if (state.timer < timepoint4) {
        state.currentImage = assets.smokinga2;
        state.currentVerticalOffset += 20;
    } else {
        state.currentImage = assets.smokinga;
    ...
    // Logika generowania dymu
    if (state.isSpawningSmoke) {
        state.smokeSpawnTimer += deltaTime;
        if (state.smokeSpawnTimer >= state.smokeSpawnInterval) {
            state.smokeSpawnTimer = 0;
            
            // --- GŁÓWNE ZMIANY W PARAMETRACH DYMU ---
            state.smokeParticles.push({
                x: canvas.width / 2 + (Math.random() - 0.5) * 280,    // Mniejszy rozrzut na boki (centrum)
                y: canvas.height + 150 + (Math.random() - 0.5) * 40, // Generowany niżej
                size: 60 + Math.random() * 140,                       // Startuje mniejszy...
                growth: 35 + Math.random() * 220,                     // ...ale BARDZO SZYBKO rośnie
                vy: -1.5 - Math.random() * 0.7,                      // Szybki ruch "do przodu" (w górę)
                vx: (Math.random() - 0.5) * 2.5,
                baseAlpha: 0.05 + Math.random() * 0.09,              // Bazowa przezroczystość
                alpha: 0,                                            // Aktualna przezroczystość (obliczana później)
                lifetime: 0,
                maxLifetime: 6.0 + Math.random() * 4.0               // Dłuższy czas życia (6-10 sekund)
            });
            // --- KONIEC ZMIAN ---
        }
    }
    
    // Aktualizacja istniejących cząsteczek dymu
    for (let i = state.smokeParticles.length - 1; i >= 0; i--) {
        const p = state.smokeParticles[i];
        p.lifetime += deltaTime;
        if (p.lifetime >= p.maxLifetime) {
            ...
            p.x += p.vx;
            p.size += p.growth * deltaTime;
            // Nieliniowe zanikanie, aby dym był widoczny dłużej
            const lifeProgress = p.lifetime / p.maxLifetime;
            p.alpha = p.baseAlpha * (1 - Math.pow(lifeProgress, 2));
        }
    }
    ...
    if (state.timer >= state.totalDuration) {
        ...
        const newThrownCigarette = {
            x: canvas.width / 2, y: canvas.height + 100,
            vx: (Math.random() - 0.5) * 15, vy: -35 - Math.random() * 10,
            angle: 0, rotationSpeed: (Math.random() - 0.5) * 0.2, gravity: 1.2,
            width: 80, height: 30
        };
        thrownCigarettes.push(newThrownCigarette);
    }
}
/**
 * Rysuje animację palenia papierosa i dym.
 */
function drawSmokingAnimation() {
    // Rysuj tylko, gdy animacja jest aktywna LUB gdy są jeszcze cząsteczki dymu
    if (!smokingAnimationState.isActive && smokingAnimationState.smokeParticles.length === 0) {
        return;
    }

    const state = smokingAnimationState;

    // Rysowanie głównego obrazka (tylko gdy animacja jest aktywna)
    if (state.isActive && state.currentImage && state.currentImage.complete) {
        const img = state.currentImage;
        const scale = 1;
        const width = img.naturalWidth * scale;
        const height = img.naturalHeight * scale;
        ...
        const drawX = (canvas.width / 2) - (width / 2) + state.currentOffsetX;
        const drawY = (canvas.height - height) + state.currentVerticalOffset + state.currentOffsetY;

        ctx.drawImage(img, drawX, drawY, width, height);
    }

    // Rysowanie dymu (zawsze, jeśli istnieją cząsteczki)
    if (assets.dust_blue && assets.dust_blue.complete && state.smokeParticles.length > 0) {
        ctx.save();
        // --- USUNIĘTO: ctx.globalCompositeOperation = 'lighter'; --- 
        // To sprawia, że dym jest siwy, a nie biały/jasny.
...
/**
 * Aktualizuje fizykę rzuconych niedopałków.
 */
function updateThrownCigarettes(deltaTime) {
    for (let i = thrownCigarettes.length - 1; i >= 0; i--) {
        const cig = thrownCigarettes[i];
        cig.vy += cig.gravity;
        cig.x += cig.vx;
        cig.y += cig.vy;
        cig.angle += cig.rotationSpeed;

        if (cig.y > canvas.height + 100) {
            thrownCigarettes.splice(i, 1);
        }
    }
}

/**
 * Rysuje wszystkie rzucone niedopałki.
 */
function drawThrownCigarettes() {
    if (thrownCigarettes.length === 0 || !assets.oldcigarette.complete) return;

    for (const cig of thrownCigarettes) {
        ctx.save();
...
function spawnSteamParticle(x, y) {
    // Stara linia (generuje 3-5 cząsteczek):
    // const particleCount = 3 + Math.floor(Math.random() * 3);

    // NOWA, ZMIENIONA LINIA (generuje 1-2 cząsteczki):
    const particleCount = 0.2;
    ...
    for (let i = 0; i < particleCount; i++) {
        steamParticles.push({
            ...
            growth: 40 + Math.random() * 180,
            alpha: 0.01 + Math.random() * 0.02,
            vy: -0.5 - Math.random() * 1.5,
            vx: (Math.random() - 0.5) * 1.0,
            lifetime: 0,
            maxLifetime: 1.5 + Math.random() * 2.0,
        });
    }
}

/**
 * Aktualizuje wszystkie aktywne cząsteczki pary.
 */
function updateSteamParticles(deltaTime) {
    for (let i = steamParticles.length - 1; i >= 0; i--) {
        const p = steamParticles[i];
        p.lifetime += deltaTime;

        if (p.lifetime >= p.maxLifetime) {
            steamParticles.splice(i, 1); // Usuń cząstkę po czasie
            continue;
        }

        // Aktualizacja pozycji, rozmiaru i przezroczystości
        p.x += p.vx;
        p.y += p.vy;
        p.size += p.growth * deltaTime;
        const lifeProgress = p.lifetime / p.maxLifetime;
        p.alpha = (1.0 - lifeProgress) * 0.3; // Zanikanie
    }
}

/**
 * Rysuje wszystkie aktywne cząsteczki pary.
 */
function drawSteamParticles() {
    if (steamParticles.length === 0 || !assets.dust_blue.complete) {
        return;
    }
    ctx.save();
    // --- ZMIANA: USUNIĘTO TĘ LINIĘ ---
...
function triggerEngineFailure() {
    // 1. Zabezpieczenie: nie uruchamiaj, jeśli silnik już jest zepsuty.
    if (gameState.isOverheating) return;

    console.log("PROCEDURA AWARII SILNIKA URUCHOMIONA!");

    // 2. Zmieniamy kluczowe stany gry.
    gameState.isOverheating = true;
    gameState.temperature = 150; // Ustawiamy temperaturę na maksimum.

    // === NOWY KOD: Aktywacja trzęsienia ekranu ===
    // Uruchamiamy wstrząsy, które są już obsługiwane przez główną pętlę gry.
    // Doda to dynamiczne trzęsienie ekranu w momencie gaśnięcia silnika.
    gameState.overheatShake.active = true;
    gameState.overheatShake.timer = 0;
    // === KONIEC NOWEGO KODU ===

    // === POPRAWKA: Usunięto bezpośrednie zarządzanie dźwiękiem ===
    // Teraz ta funkcja TYLKO ustawia stan. Główna pętla w `manageGameSounds`
    // zobaczy, że `isOverheating` jest `true` i sama wyciszy odpowiednie dźwięki.
    // To rozwiązuje problem z odtwarzającym się w tle dźwiękiem.
    
    // 3. Odtworzenie dźwięku awarii (boom.mp3) - ale tylko raz!
    if (!gameState.overheatSoundPlayed && audio.unlocked && audio.boom) {
        audio.boom.currentTime = 0;
        audio.boom.volume = 0.7;
        audio.boom.play().catch(() => {});
        gameState.overheatSoundPlayed = true; // Ustawiamy flagę, by nie grać dźwięku w pętli.
    }
}

function updateWaterDropsAndCooling(deltaTime) {
    let wasWaterOnEngineThisFrame = false;
    const gravity = 0.25;

    const engineHorizontalRange = { x: 700, width: 750 };
    const baseSurfaceY = 450;
    const surfaceAmplitude = 100;
    const surfaceFrequency = 0.01;

    for (let i = waterDrops.length - 1; i >= 0; i--) {
        const drop = waterDrops[i];

        if (currentView === 'engine' &&
            drop.x > engineHorizontalRange.x && drop.x < engineHorizontalRange.x + engineHorizontalRange.width)
        {
            const surfaceY = baseSurfaceY + Math.sin(drop.x * surfaceFrequency) * surfaceAmplitude;

            if (drop.y >= surfaceY) {
                wasWaterOnEngineThisFrame = true;
                
                // =================================================================
                // === GŁÓWNA ZMIANA JEST TUTAJ ===
                // =================================================================

                // 1. Zawsze zliczamy kroplę, która trafiła w silnik.
                engineViewState.coolingDropsCount++;

                // 2. Definiujemy próg, po którym zaczyna się chłodzenie.
                const COOLING_THRESHOLD = 120;

                // 3. Sprawdzamy, czy próg został przekroczony.
                if (engineViewState.coolingDropsCount > COOLING_THRESHOLD) {
                    // Jeśli tak, uruchamiamy logikę zmiany temperatury.
                    if (gameState.temperature < 110) {
                        triggerEngineFailure();
                    } else {
                        gameState.temperature = Math.max(105, gameState.temperature - 0.03);
                    }
                }
                
                // 4. Efekty wizualne (para) i usunięcie kropli dzieją się ZAWSZE,
                //    niezależnie od tego, czy silnik się schładza.
                spawnSteamParticle(drop.x, drop.y);
                waterDrops.splice(i, 1);
                continue; // Przechodzimy do następnej kropli
        ...
        // Standardowa fizyka kropli (bez zmian)
        drop.lifetime += deltaTime;
        if (drop.lifetime >= drop.maxLifetime) {
            waterDrops.splice(i, 1);
            continue;
    ...
    return wasWaterOnEngineThisFrame;
}
/**
 * Zarządza odtwarzaniem i pauzowaniem dźwięku smażenia.
 * @param {boolean} shouldBePlaying - Czy dźwięk powinien grać w tej klatce.
 */
function manageFryingSound(shouldBePlaying) {
    if (!audio.unlocked || !audio.frying) return;

    if (shouldBePlaying) {
        if (audio.frying.paused) {
            engineFryingState.isPlaying = true;
            audio.frying.play().catch(e => console.warn("Nie udało się odtworzyć frying.mp3", e));
        }
    } else {
        if (!audio.frying.paused) {
            engineFryingState.isPlaying = false;
            audio.frying.pause();
        }
    }
}
...
function spawnNewFly() {
    // Tworzymy obiekt nowej muchy, kopiując strukturę starego `stanMuchy`
    const nowaMucha = {
        x: obszarSzyby.x + Math.random() * obszarSzyby.width,
        y: obszarSzyby.y + Math.random() * obszarSzyby.height,
        stan: 'chodzi', // Możliwe stany: 'chodzi', 'lata', 'stoi', 'uderzona'
        kat: 0,
        celX: 0,
        celY: 0,
        timerZmianyStanu: 5 + Math.random() * 15,
        minCzasChodzenia: 8,
        maxCzasChodzenia: 20,
        minCzasLatania: 5,
        maxCzasLatania: 32,
        czasRuchu: Math.random() * 100, // Losowa faza startowa
        predkoscChodzenia: 45,
        amplitudaLukuChodzenia: 0.8,
        czestotliwoscLukuChodzenia: 3.0,
        wariacjaPredkosciChodzenia: 0.3,
        predkoscLatania: 580,
        amplitudaLukuLatania: 0.8,
        czestotliwoscLukuLatania: 4.2,
        wariacjaPredkosciLatania: 0.7,
        czasDoZatrzymania: 1 + Math.random() * 3,
        czasZatrzymania: 0,
        minCzasDoZatrzymania: 1,
        maxCzasDoZatrzymania: 4,
        minCzasPostoju: 0.5,
        maxCzasPostoju: 2.5,
        // Właściwości dla stanu 'uderzona'
        vy: 0, // Prędkość pionowa spadania
        predkoscObrotu: 0
    };
    wybierzNowyCelDlaMuchy(nowaMucha, true); // Wybieramy pierwszy cel dla nowej muchy
    muchy.push(nowaMucha);
}
...
const roadsideObjects = { 
    ...
    lanterns: [],
    trafficCones: [],
    fences: [], // <--- DODAJ TĘ LINIĘ
    slowOpponents: []
};

const fenceSpawner = {
    // Kiedy ma pojawić się następny PAS płotu
    nextSpawnDistance: 30 + Math.random() * 80, 
    
    // Zmienne do zarządzania aktualnie generowanym pasem płotu
    inStrip: false,              // Czy jesteśmy w trakcie generowania pasa
    stripPostsRemaining: 0,      // Ile słupków zostało do wygenerowania w tym pasie
    stripSide: 'left',           // Po której stronie drogi jest obecny pas
    lastPostSliceIndex: 0        // Pozycja ostatniego słupka, by stawiać kolejne blisko
};
...
const trafficConeSpawner = {
    
    nextSpawnDistance: 20 + Math.random() * 70, // Kiedy pojawi się pierwsza grupa
    minInterval: 10,     // Minimalny odstęp między GRUPAMI
    maxInterval: 800,    // Maksymalny odstęp między GRUPAMI
    maxOffsetFromEdge: 60 // Zwiększ tę wartość (np. do 120), aby pachołki były bardziej rozrzucone
};
...
function updateTrafficConesAndSpawner(speedKmH) {
    if (totalDistanceTraveled >= trafficConeSpawner.nextSpawnDistance) {
        
        const groupSize = Math.floor(Math.random() * 3) + 2; // Losowo od 2 do 4
        const side = Math.random() < 0.5 ? 'left' : 'right';
        const baseSpacing = 35; // Bazowy odstęp "w głąb" między pachołkami

        for (let i = 0; i < groupSize; i++) {
            // Losowe odsunięcie na boki (od 0 do wartości maksymalnej)
            const randomLateralOffset = ((Math.random() - 0.5) * 2) * trafficConeSpawner.maxOffsetFromEdge;
            // Losowe odsunięcie w głąb sceny, aby złamać prostą linię
            const randomDepthOffset = Math.random() * baseSpacing * 0.8;

            const newCone = {
                // Każdy kolejny pachołek jest dalej, z dodaną losowością
                sliceIndex: gameState.roadsideMaxSlices - (i * baseSpacing) - randomDepthOffset,
                side: side,
                image: assets.trafficCone,
                sizeVariation: 0.25,
                // Zapisujemy unikalne odsunięcie na boki dla tego pachołka
                lateralOffset: randomLateralOffset
            };
            roadsideObjects.trafficCones.push(newCone);
        }

        const randomInterval = trafficConeSpawner.minInterval + Math.random() * (trafficConeSpawner.maxInterval - trafficConeSpawner.minInterval);
        trafficConeSpawner.nextSpawnDistance = totalDistanceTraveled + randomInterval;
    }

    // Aktualizacja pozycji (bez zmian)
    const speedFactor = speedKmH / gameState.maxSpeedKmH;
    for (let i = roadsideObjects.trafficCones.length - 1; i >= 0; i--) {
        const cone = roadsideObjects.trafficCones[i];
        cone.sliceIndex -= speedFactor * (0.10 + 79.4 * (1 - cone.sliceIndex / gameState.roadsideMaxSlices));
        if (cone.sliceIndex < -10) {
            roadsideObjects.trafficCones.splice(i, 1);
        }
    }
}

/**
 * Rysuje pachołki drogowe na ekranie.
 */

// --- POPRAWIONA WERSJA: Funkcja do aktualizacji i spawnowania płotów ---
function updateFencesAndSpawner(speedKmH) {
    // --- CZĘŚĆ 1: SPAWNER ---
    // Sprawdzamy, czy dojechaliśmy do miejsca, gdzie ma zacząć się nowy pas płotu
    if (!fenceSpawner.inStrip && totalDistanceTraveled >= fenceSpawner.nextSpawnDistance) {
        
        // Zaczynamy generowanie nowego pasa
        fenceSpawner.inStrip = true;
        
        // Losujemy długość pasa w METRACH (np. od 200 do 400 metrów)
        const stripLengthInMeters = 20 + Math.random() * 170;
        fenceSpawner.stripEndsAtDistance = totalDistanceTraveled + stripLengthInMeters;

        // Ustalamy, kiedy postawić pierwszy słupek
        fenceSpawner.distanceOfNextPost = totalDistanceTraveled;
        
        // Losujemy stronę drogi
        fenceSpawner.stripSide = Math.random() < 0.5 ? 'left' : 'right';

        // --- ZMIANA: Losujemy teksturę dla całego nadchodzącego pasa ---
        if (assets.fences.length > 0) {
            const randomIndex = Math.floor(Math.random() * assets.fences.length);
            fenceSpawner.currentStripTexture = assets.fences[randomIndex];
        } else {
            fenceSpawner.currentStripTexture = null; // Zabezpieczenie na wypadek braku tekstur
        }
        // --- KONIEC ZMIANY ---
    }

    // Jeśli jesteśmy w trakcie generowania pasa...
    if (fenceSpawner.inStrip) {
        // ...i przejechaliśmy dystans potrzebny do postawienia kolejnego słupka...
        if (totalDistanceTraveled >= fenceSpawner.distanceOfNextPost) {
            
            // Tworzymy NOWY SŁUPEK na horyzoncie
            const newFencePost = {
                sliceIndex: gameState.roadsideMaxSlices,
                side: fenceSpawner.stripSide,
                sizeVariation: 0.2,
                // --- ZMIANA: Każdy słupek dostaje teksturę wylosowaną dla całego pasa ---
                image: fenceSpawner.currentStripTexture 
            };
            roadsideObjects.fences.push(newFencePost);

            // Ustalamy dystans, przy którym ma powstać NASTĘPNY słupek
            const spacing = 1.3; // Odstęp w metrach między słupkami
            fenceSpawner.distanceOfNextPost += spacing;
        }

        // Sprawdzamy, czy pas płotu się już skończył
        if (totalDistanceTraveled >= fenceSpawner.stripEndsAtDistance) {
            fenceSpawner.inStrip = false;
            // Ustalamy, kiedy ma się pojawić kolejny pas (duży, losowy odstęp)
            fenceSpawner.nextSpawnDistance = totalDistanceTraveled + 90 + Math.random() * 50;
        }
    }

    // --- CZĘŚĆ 2: AKTUALIZACJA POZYCJI (bez zmian) ---
    const speedFactor = speedKmH / gameState.maxSpeedKmH;
    for (let i = roadsideObjects.fences.length - 1; i >= 0; i--) {
        const fence = roadsideObjects.fences[i];
        const t = 1 - fence.sliceIndex / gameState.roadsideMaxSlices;
        const speedMultiplier = 0.10 + 79.4 * t;
        fence.sliceIndex -= speedFactor * speedMultiplier;

        if (fence.sliceIndex < -10) {
            roadsideObjects.fences.splice(i, 1);
        }
    }
}

function drawFences() {
    // --- ZMIANA: Sprawdzamy, czy tablica płotów w ogóle istnieje i nie jest pusta ---
    if (!assets.fences || assets.fences.length === 0 || roadsideObjects.fences.length === 0) {
        return;
    }

    const roadWidthTop = canvas.width * 0.001;
    const roadWidthBottom = canvas.width * 2.92;

    roadsideObjects.fences.sort((a, b) => b.sliceIndex - a.sliceIndex);

    for (const fence of roadsideObjects.fences) {
        // --- ZMIANA: Sprawdzamy, czy TEN konkretny słupek ma załadowany obrazek ---
        if (!fence.image || !fence.image.complete || fence.image.naturalWidth === 0) {
            continue; // Pomiń rysowanie, jeśli tekstura nie jest gotowa
        }
        
        const t = 1 - fence.sliceIndex / gameState.roadsideMaxSlices;
        if (t < 0) continue;

        const y = (canvas.height * 0.35) + t * (canvas.height * 0.65);
        const width = roadWidthTop + (roadWidthBottom - roadWidthTop) * t;
        const lateralOffset = -(gameState.playerX / gameState.maxOffset) * 1930 * t;
        
        const roadLeftX = (canvas.width - width) / 2 + lateralOffset;
        const roadRightX = roadLeftX + width;
        
        const baseSize = width * 0.052;
        const objSize = baseSize * fence.sizeVariation;

        const offsetFromRoad = 50; 
        let x;
        if (fence.side === "left") {
            x = roadLeftX + (offsetFromRoad * t); 
        } else {
            x = roadRightX - (offsetFromRoad * t);
        }

        let horizontalScale = 1;
        const minScale = 0.19;

        if (fence.side === 'right' && gameState.playerX > 0) {
            const progress = gameState.playerX / gameState.maxOffset;
            horizontalScale = 1 + progress * (1.0 - minScale) * -0.1;
        } else if (fence.side === 'left' && gameState.playerX < 0) {
            const progress = Math.abs(gameState.playerX) / gameState.maxOffset;
            horizontalScale = 1 + progress * (1.0 - minScale) * -0.1;
        }
        horizontalScale = Math.max(minScale, horizontalScale);

        // --- ZMIANA: Używamy fence.image zamiast globalnego assets.fence ---
        const baseDrawWidth = Math.round(objSize * 5.5);
        const finalDrawWidth = baseDrawWidth * horizontalScale;
        const finalDrawHeight = finalDrawWidth * (fence.image.naturalHeight / fence.image.naturalWidth);
        
        const drawX = Math.round(x - finalDrawWidth / 2);
        const drawY = Math.round(y - finalDrawHeight);

        ctx.save();

        if (fence.side === 'right') {
            ctx.translate(drawX + finalDrawWidth, drawY);
            ctx.scale(-1, 1);
            ctx.drawImage(fence.image, 0, 0, finalDrawWidth, finalDrawHeight); // Używamy fence.image
        } else {
            ctx.drawImage(fence.image, drawX, drawY, finalDrawWidth, finalDrawHeight); // Używamy fence.image
        }
...
function drawTrafficCones() {
    if (!assets.trafficCone.complete || !assets.trafficCone.naturalWidth === 0 || roadsideObjects.trafficCones.length === 0) {
        return;
    }

    const roadWidthTop = canvas.width * 0.001;
    const roadWidthBottom = canvas.width * 1.7;

    roadsideObjects.trafficCones.sort((a, b) => b.sliceIndex - a.sliceIndex);

    for (const cone of roadsideObjects.trafficCones) {
        const t = 1 - cone.sliceIndex / gameState.roadsideMaxSlices;
        if (t < 0) continue;

        const y = (canvas.height * 0.35) + t * (canvas.height * 0.65);
        const width = roadWidthTop + (roadWidthBottom - roadWidthTop) * t;
        const lateralOffset = -(gameState.playerX / gameState.maxOffset) * 1630 * t;
        
        const roadLeftX = (canvas.width - width) / 2 + lateralOffset;
        const roadRightX = roadLeftX + width;
        
        const baseSize = width * 0.22;
        const objSize = baseSize * cone.sizeVariation;

        // --- GŁÓWNA ZMIANA LOGIKI POZYCJONOWANIA ---

        // 1. Definiujemy bazowe odsunięcie w głąb drogi. Zwiększ tę wartość, by były bliżej środka.
        const baseOffsetInsideRoad = 250; 
        
        // 2. Pobieramy dodatkowe, losowe odsunięcie dla tego pachołka
        const randomOffset = cone.lateralOffset || 0;
        
        // 3. Obliczamy finalne, całkowite odsunięcie
        const totalOffset = baseOffsetInsideRoad + randomOffset;

        let x;
        if (cone.side === "left") {
            // Znak PLUS przesuwa pachołek w prawo (do wewnątrz lewego pasa)
            x = roadLeftX + (totalOffset * t); 
        } else {
            // Znak MINUS przesuwa pachołek w lewo (do wewnątrz prawego pasa)
            x = roadRightX - (totalOffset * t);
        }
        // --- KONIEC ZMIAN ---

        const drawWidth = Math.round(objSize * 2);
        const drawHeight = Math.round(objSize * 2 * (assets.trafficCone.naturalHeight / assets.trafficCone.naturalWidth));
        const drawX = Math.round(x - drawWidth / 2);
        const drawY = Math.round(y - drawHeight);

        ctx.drawImage(cone.image, drawX, drawY, drawWidth, drawHeight);
    }
}
...
function updateKeyrings(deltaTime) {
    // Iterujemy przez każdy breloczek w naszym stanie
    for (const keyring of keyringState.keyrings) {
        // Siły działające na breloczek (skopiowane z `updateHanger`)
        const turnForce = -gameState.tilt * 0.0008;
        const shakeForce = -gameState.shakeX * 0.002;
        const restoringForce = -keyring.angle * keyring.stiffness;
        
        const acceleration = restoringForce + turnForce + shakeForce;
        
        keyring.velocity += acceleration;
        keyring.velocity *= keyring.damping; // Tłumienie ruchu
        
        // Dodatkowe, losowe drgania na trawie
        if (gameState.isInGrass && Math.abs(gameState.shakeX) > 1) {
            if (Math.random() > 0.95) {
                keyring.velocity += (Math.random() - 0.5) * 0.03;
            }
        }
        
        keyring.angle += keyring.velocity;
    }
}

/**
 * Rysuje oba breloczki na ekranie.
 */
function drawKeyrings() {
    // Ta funkcja musi być wywołana wewnątrz transformacji kokpitu, aby breloczki poruszały się razem z nim.
    
    for (let i = 0; i < keyringState.keyrings.length; i++) {
        const keyring = keyringState.keyrings[i];

        // --- DODAJ TEN CONSOLE.LOG ---

        if (!keyring.image || !keyring.image.complete || keyring.image.naturalWidth === 0) {
             continue; // Jeśli obrazek jest niezaładowany, pomiń rysowanie
        }

        ctx.save();
        
        ctx.translate(keyringState.anchorX, keyringState.anchorY);
        ctx.rotate(keyring.angle);
        
        ctx.drawImage(
            keyring.image,
            -keyring.size.width / 2,
            0,
            keyring.size.width,
            keyring.size.height
        );
...
const audio = { 
     washer: new Audio("sound/washer.mp3"), // <-- NOWA LINIA: Dźwięk spryskiwaczy
     flying: new Audio("sound/flying.mp3"), // <-- NOWA LINIA
  menuopen: new Audio("sound/menuopen.mp3"),
  frying: new Audio("sound/frying.mp3"), // <-- NOWY KOD
  ...
  viewIn: new Audio("sound/in.mp3"),      
  viewOut: new Audio("sound/out.mp3"),    
  c_engineidle: new Audio("sound/c_engineidle.mp3"), 
  screw: new Audio("sound/screw.mp3"),             
  pouring: new Audio("sound/pouring.mp3"),         
  openingbeer: new Audio("sound/openingbeer.mp3"), 
  drinking: new Audio("sound/drinking.mp3"),       
  dipstick: new Audio("sound/dipstick.mp3"),
  objecthit: new Audio("sound/objecthit.mp3"),
  lighter: new Audio("sound/lighter.mp3"), // <--- NOWY DŹWIĘK
  // --- NOWY KOD ---
  turningoff: new Audio("sound/turningoff.mp3"), // Dźwięk gaśnięcia silnika
  ignition: new Audio("sound/ignition.mp3"),     // Dźwięk próby odpalenia
  // --- KONIEC NOWEGO KODU ---
  unlocked: false 
};

// ... reszta ustawień audio ...

audio.lighter.loop = false; // <--- UPEWNIJ SIĘ, ŻE DŹWIĘK NIE JEST W PĘTLI
if (audio.flying) audio.flying.loop = true;
audio.washer.loop = false; // <-- NOWA LINIA
audio.turningoff.loop = false;
audio.ignition.loop = true; // Ten dźwięk musi być w pętli


if (audio.frying) audio.frying.loop = true;
...
audio.dipstick.loop = false;    // <--- DODAJ TĘ LINIĘ
// --- KONIEC NOWEGO KODU ---
...
function unlockAudio() {
  const soundsToUnlock = [
     audio.objecthit, // <--- DODAJ TĘ LINIĘ
    audio.background, audio.carbackground, audio.idle, audio.ac, audio.ac1,  audio.washer,audio.flying,
    ...
    audio.radio, audio.rewind, audio.scroll,  audio.frying,
    ...
    audio.openingbeer, audio.drinking, audio.dipstick, audio.lighter,
    // --- NOWY KOD ---
    audio.turningoff, audio.ignition // <-- Dodaj te dwa dźwięki tutaj
    // --- KONIEC NOWEGO KODU ---
  ];
...
function managePouringSound() {
    ...
    // Definiujemy progi wylewania (ten sam dla oleju, piwa i wody)
    const POURING_THRESHOLD_ANGLE = 45 * (Math.PI / 180);

    // Sprawdzamy, czy olej się leje
    const isOilPouring = heldOilState.isHeld && Math.abs(heldOilState.angle) > POURING_THRESHOLD_ANGLE;

    // Sprawdzamy, czy piwo się leje
    const isBeerPouring = heldBeerState.isHeld && Math.abs(heldBeerState.angle) > POURING_THRESHOLD_ANGLE;
    
    // --- NOWY KOD: Sprawdzamy, czy woda się leje ---
    const isWaterPouring = heldJugState.isHeld && Math.abs(heldJugState.angle) > POURING_THRESHOLD_ANGLE;

    // Warunek końcowy: czy cokolwiek się leje?
    const isPouring = isOilPouring || isBeerPouring || isWaterPouring;

    if (isPouring) {
        // Jeśli warunki są spełnione, a dźwięk nie gra, włącz go.
        if (audio.pouring.paused) {
            audio.pouring.currentTime = 0; // Opcjonalnie resetuj, by dźwięk zaczynał się od nowa
            audio.pouring.play().catch(e => console.warn("Nie udało się odtworzyć pouring.mp3", e));
        }
    } else {
        // Jeśli warunki nie są spełnione, a dźwięk wciąż gra, zatrzymaj go.
        if (!audio.pouring.paused) {
            audio.pouring.pause();
        }
    }
...
function updateBuildings(speedKmH) {
    // 1. Sprawdzamy, czy mamy jakiekolwiek obrazki budynków do użycia.
    // Tworzymy jedną, wspólną pulę wszystkich dostępnych budynków.
    const allBuildingImages = assets.buildingImages.concat(assets.specialBuildingImages);
    if (allBuildingImages.length === 0) return; // Jeśli nie ma żadnych, kończymy.

    const speedFactor = speedKmH / gameState.maxSpeedKmH;
    
    // 2. Logika spawnera (zmniejszanie dystansu do następnego spawnu).
    buildingSpawner.nextSpawnDistance -= speedFactor * 80;

    if (buildingSpawner.nextSpawnDistance <= 0) {
        // 3. Wybieramy losowy budynek z połączonej puli.
        const randomImage = allBuildingImages[Math.floor(Math.random() * allBuildingImages.length)];
        
        // 4. Tworzymy nowy obiekt budynku (logika bez zmian).
        const newBuilding = {
            sliceIndex: gameState.roadsideMaxSlices,
            side: Math.random() < 0.5 ? "left" : "right",
            image: randomImage,
            sizeVariation: 0.68 + Math.random() * 0.1,
            xOffset: 200 + Math.random() * 600
        };
        roadsideObjects.buildings.push(newBuilding);
        
        // 5. Ustawiamy JEDNOLITĄ, losową odległość do następnego budynku, bez względu na "miasto".
        // Używamy większych wartości, aby budynki nie pojawiały się zbyt często.
        buildingSpawner.nextSpawnDistance = 8000 + Math.random() * 15000;
    }

    // 6. Aktualizujemy pozycję wszystkich istniejących budynków (logika bez zmian).
    for (let i = roadsideObjects.buildings.length - 1; i >= 0; i--) {
        const building = roadsideObjects.buildings[i];
        const t = 1 - building.sliceIndex / gameState.roadsideMaxSlices;
        const speedMultiplier = 0.10 + 75.4 * t;
        building.sliceIndex -= speedFactor * speedMultiplier;
        if (building.sliceIndex < -10) {
            roadsideObjects.buildings.splice(i, 1);
        }
    }
...
function updateRadioShake(deltaTime) {
    if (radioState.shakeTimer > 0) {
        // Zmniejszamy timer
        radioState.shakeTimer -= deltaTime;
        
        // Obliczamy losowe przesunięcie w zakresie [-intensity, +intensity]
        const intensity = radioState.shakeIntensity;
        radioState.shakeOffsetX = (Math.random() - 0.5) * 2 * intensity;
        radioState.shakeOffsetY = (Math.random() - 0.5) * 2 * intensity;
    } else {
        // Jeśli czas minął, zerujemy przesunięcie
        radioState.shakeOffsetX = 0;
        radioState.shakeOffsetY = 0;
    }
}
...
// --- NOWY KOD: FUNKCJA RYSUJĄCA NOWY, ANIMOWANY LICZNIK ---
function drawHoverCounter(x, y) {
    // --- ZMIANA 1: Wczesne wyjście, jeśli licznik jest niewidoczny ---
    if (hoverCounterState.alpha <= 0) {
        return; // Optymalizacja: nie rysuj nic, jeśli jest w pełni przezroczysty
    }
    ...
    // --- ZMIANA 2: Zapisanie stanu i ustawienie przezroczystości ---
    ctx.save();
    ctx.globalAlpha = hoverCounterState.alpha; // Ustawiamy aktualną przezroczystość

    // Reszta funkcji rysującej pozostaje bez zmian...
    const hudWidth = 380;
    const hudHeight = 295;
    const hudX = x - hudWidth / 2;
    const hudY = y - hudHeight / 2;
    ...
    const startAngleRad = -0.72 * Math.PI;
    const rangeAngleRad = 1.1 * Math.PI;
    ...
    ctx.drawImage(assets.counterNeedle, -needleWidth / 2, -needleHeight + 10, needleWidth, needleHeight);
    ctx.restore();

    // ...rysowanie licznika kilometrów...
    ctx.save();
    const { displayDigits, rollProgress, isRolling } = odometerState;
    ctx.font = "bold 22px 'DSDigi'";
    ...
    for (let i = 0; i < 5; i++) {
        ...
        if (isRolling[i]) {
            ...
            ctx.save();
            ctx.globalAlpha = (1.0 - animationProgress) * 0.7 * hoverCounterState.alpha; // Mnożymy przez alfę licznika
            ctx.fillText(oldDigitChar, drawX, odometerBaseY + oldDigitYOffset);
            ctx.restore();
            
            ctx.save();
            ctx.globalAlpha = animationProgress * 0.85 * hoverCounterState.alpha; // Mnożymy przez alfę licznika
            ctx.fillText(newDigitChar, drawX, odometerBaseY + newDigitYOffset);
            ctx.restore();
        } else {
            ctx.fillText(newDigitChar, drawX, odometerBaseY);
    ...
    // --- ZMIANA 3: Przywrócenie stanu kontekstu ---
    ctx.restore(); // To jest bardzo ważne, aby nie wpływać na inne elementy
}

function drawCigaretteBox() {
    // Rysuj tylko w widoku kokpitu
    if (currentView !== 'cockpit') {
        return;
    }
    const { x, y, width, height } = cigaretteBoxState.rect;
    ctx.fillStyle = "rgba(220, 20, 20, 0)"; // Ciemniejszy czerwony z lekką przezroczystością
    ctx.fillRect(x, y, width, height);
}


/**
 * Aktualizuje fizykę trzymanego papierosa.
 */
function updateHeldCigarette(deltaTime) {
    if (!heldCigaretteState.isHeld) return;

    const restoringForce = -heldCigaretteState.angle * heldCigaretteState.stiffness;
    heldCigaretteState.angleVelocity += restoringForce;
    heldCigaretteState.angleVelocity *= heldCigaretteState.damping;
    heldCigaretteState.angle += heldCigaretteState.angleVelocity;
}

/**
 * Rysuje trzymanego papierosa przy kursorze.
 */
function drawHeldCigarette() {
    if (!heldCigaretteState.isHeld || !assets.cigarette || !assets.cigarette.complete) {
        return;
    }
    
    ctx.save();
    ctx.translate(heldCigaretteState.x, heldCigaretteState.y);
    ctx.rotate(heldCigaretteState.angle);
    ctx.drawImage(
        assets.cigarette,
        -heldCigaretteState.width / 2, 
        -heldCigaretteState.height / 2, 
        heldCigaretteState.width, 
        heldCigaretteState.height
    );
    ctx.restore();
}
...
function drawRadioControls() {
    ...
    // Rysowanie niewidzialnych przycisków (bez zmian)
    ...
    // Rysowanie obrazka radia
    if (radioState.opacity > 0 && assets.radio.complete) {
        ctx.save();
        ctx.globalAlpha = radioState.opacity; 
        const imgPos = radioState.imagePosition;

        // MODYFIKACJA: Dodajemy przesunięcie z drgania do pozycji rysowania
        const drawX = imgPos.x + radioState.shakeOffsetX;
        const drawY = imgPos.y + radioState.shakeOffsetY;
        
        ctx.drawImage(assets.radio, drawX, drawY, imgPos.width, imgPos.height);
        ctx.restore();
    }
}
...
function drawCockpit() {
  ...
  ctx.save(); 
  drawWipers();
   drawWashingEffect(); // <-- DODAJ TĘ LINIĘ
   drawSplashEffect(); // <-- DODAJ TĘ LINIĘ
  drawMud();
  ctx.restore(); 

  // 1. Rysujemy bazowy kokpit (tło dla naszych breloczków)
  ctx.drawImage(assets.cockpit, 0, 0, canvas.width, canvas.height);
  
  drawMuchy();
  // 2. Rysujemy wszystkie elementy, które są "na" kokpicie
  drawInteriorLight();
  drawSunVisors();
  drawMirror();
  drawPedals();
  ...
  // --- POPRAWKA TUTAJ ---
  // Rysujemy breloczki w tym miejscu, aby były NA kokpicie, a nie pod nim.
  // Zostały przeniesione z miejsca, gdzie był rysowany czerwony kwadrat.
  drawKeyrings();
  // --- KONIEC POPRAWKI ---
  ...
  // To jest ctx.restore() dla głównej transformacji kokpitu
  ctx.restore();
}
...
function joltKeyrings() {
    for (const keyring of keyringState.keyrings) {
        // Dodajemy losową, silną prędkość kątową
        keyring.velocity += (Math.random() - 0.5) * 0.15; // Możesz dostosować siłę (0.15)
    }
}
...
canvas.addEventListener("click", (e) => {
    ...
    const hitboxSize = 55; // Mały hitbox, trudniej trafić

    // Pętla sprawdzająca każdą muchę
    for (const mucha of muchy) {
        // Sprawdzamy tylko muchy, które nie są już uderzone
        if (mucha.stan !== 'uderzona') {
            const hitbox = {
                x: mucha.x - hitboxSize / 2,
                y: mucha.y - hitboxSize / 2,
                width: hitboxSize,
                height: hitboxSize
            };

            if (isMouseOver(mousePos, hitbox)) {
                // Trafienie!
                mucha.stan = 'uderzona';
                mucha.vy = -5; // Lekkie podbicie w górę
                mucha.predkoscObrotu = (Math.random() - 0.5) * 50 + 25; // Szybkie kręcenie się

                // Odtwórz cichy dźwięk uderzenia
                if (audio.unlocked && audio.hitboom) {
                    audio.hitboom.currentTime = 0;
                    audio.hitboom.volume = 0.18; // Cicho
                    audio.hitboom.play().catch(err => console.warn("Nie udało się odtworzyć hitboom.mp3", err));
                }

                // Zatrzymaj dźwięk latania, jeśli leciała
                if (audio.unlocked && audio.flying && !audio.flying.paused) {
                    audio.flying.pause();
                }

                console.log("Trafiono muchę!");
                return; // Zakończ pętlę i cały listener, aby nie aktywować innych kliknięć
            }
        }
    }

    for (const visor of sunVisorState.visors) {
        // Klikalny obszar to zawsze pozycja domyślna (nieprzesunięta)
        const clickArea = visor.rect; 

        if (isMouseOver(mousePos, clickArea)) {
            // Przełącz stan blendy
            visor.isFlipped = !visor.isFlipped;
            // Uruchom animację trzęsienia
            visor.shakeTimer = visor.shakeDuration;
            ...
            // Zakończ, aby nie aktywować innych klikalnych elementów (np. lusterka)
            return; 
        }
    }
...
function handleWindowScroll(e) {
    ...
    // <<< NOWA LOGIKA BLOKOWANIA - START >>>
    // Jeśli szyba jest na samej górze (offset = 0) i próbujemy kręcić w górę (delta < 0), zablokuj.
    if (windowState.glassTargetYOffset <= 0 && delta < 0) {
        return; // Zakończ funkcję, nic nie rób
    }
    // Jeśli szyba jest na samym dole i próbujemy kręcić w dół (delta > 0), zablokuj.
    if (windowState.glassTargetYOffset >= windowState.glassMaxDownOffset && delta > 0) {
        return; // Zakończ funkcję, nic nie rób
    }
    // <<< NOWA LOGIKA BLOKOWANIA - KONIEC >>>

    const scrollStep = 50; 
    let newTargetY = windowState.glassTargetYOffset;

    if (delta > 0) { // Kręcenie w dół
        newTargetY += scrollStep;
    } else { // Kręcenie w górę
        newTargetY -= scrollStep;
    }
...
window.addEventListener('wheel', (e) => {
    const mousePos = getCanvasMousePos(e.clientX, e.clientY);
     ...
     if (heldJugState.isHeld) {
        e.preventDefault();
        const direction = Math.sign(e.deltaY);
        heldJugState.angleVelocity += direction * heldJugState.scrollTiltAmount;
    ...
    if (currentView === 'cockpit') {
        ...
        if (isMouseOver(mousePos, windowState.buttonPosition)) {
            // <<< NOWA LOGIKA - START >>>
            // 1. Zapisujemy pozycję szyby PRZED próbą jej ruszenia.
            const oldTargetY = windowState.glassTargetYOffset;

            // 2. Wywołujemy funkcję, która spróbuje poruszyć szybą (ale może zostać zablokowana).
            handleWindowScroll(e);

            // 3. Sprawdzamy, czy pozycja docelowa się zmieniła.
            if (oldTargetY !== windowState.glassTargetYOffset) {
                // 4. Jeśli tak, to znaczy, że ruch był dozwolony,
                //    więc uruchamiamy animację korbki i dźwięk.
    ...
    } else if (currentView === 'trunk') { // <-- DODAJ TEN BLOK
            if (isMouseOver(mousePos, brakeCapState)) {
                e.preventDefault();
                const delta = Math.sign(e.deltaY); 
                let soundShouldPlay = false;

                if (delta > 0) { // Scroll w dół (wkręcanie)
                    if (brakeCapState.unscrewProgress > 0) {
                        soundShouldPlay = true;
                    }
                    brakeCapState.unscrewProgress -= brakeCapState.scrollSensitivity * Math.abs(e.deltaY);
                } 
                else { // Scroll w górę (odkręcanie)
                    if (brakeCapState.unscrewProgress < brakeCapState.maxUnscrewProgress) {
                        soundShouldPlay = true;
                    }
                    brakeCapState.unscrewProgress += brakeCapState.scrollSensitivity * Math.abs(e.deltaY);
                }
                
                brakeCapState.unscrewProgress = Math.max(0, Math.min(brakeCapState.maxUnscrewProgress, brakeCapState.unscrewProgress));
...
function updateWashingEffect(deltaTime) {
    const state = washingEffectState;
    if (!state.isActive) return;

    if (state.isFadingIn) {
        // Faza 1: Pojawianie się
        state.alpha += state.fadeInSpeed * deltaTime;
        if (state.alpha >= state.maxAlpha) {
            state.alpha = state.maxAlpha;
            state.isFadingIn = false; // Przełącz na fazę znikania
        }
    } else {
        // Faza 2: Znikane
        state.alpha -= state.fadeOutSpeed * deltaTime;
        if (state.alpha <= 0) {
            state.alpha = 0;
            state.isActive = false; // Zakończ i zresetuj efekt
        }
    }
}

// --- NOWY KOD: Funkcja rysująca efekt rozmazywania na szybie ---
function drawWashingEffect() {
    // Rysuj tylko, jeśli efekt jest widoczny i grafika załadowana
    if (washingEffectState.alpha <= 0 || !assets.washing || !assets.washing.complete) {
        return;
    }

    // Pobieramy pozycję i wymiary z obiektu mudState, aby były identyczne
    const { x, y, width, height } = mudState.position;

    ctx.save();
    ctx.globalAlpha = washingEffectState.alpha;
    // Rysujemy obrazek w dokładnie tym samym miejscu i rozmiarze co błoto
    ctx.drawImage(assets.washing, x, y, width, height);
    ctx.restore();
}

function updateWipers(deltaTime) {
    if (wipersState.active) {
        // --- NOWA LOGIKA: Uruchamianie efektu rozmazania ---
        // Jeśli są krople na szybie i efekt nie jest jeszcze aktywny, uruchom go.
        if (spryskiwaczeState.krople.length > 0 && !washingEffectState.isActive) {
            washingEffectState.isActive = true;
            washingEffectState.isFadingIn = true; // Rozpocznij od fazy pojawiania się
        }
        // --- KONIEC NOWEJ LOGIKI ---

        // Logika ruchu wycieraczek (pozostaje bez zmian)
        ...
        // Logika powrotu wycieraczek (pozostaje bez zmian)
...
function updateWorldObjects(deltaTime, speedKmH) {
    ...
    updateCityStatus(); updatePickets(speedKmH); updateSignSpawner();updateLanternSpawner(); updateFencesAndSpawner(speedKmH);updateTrafficConesAndSpawner(speedKmH); // <--- DODAJ TĘ LINIĘupdateTrafficConesAndSpawner(speedKmH);updateFencesAndSpawner(speedKmH); // <--- DODAJ TĘ LINIĘ
...
function manageGameSounds() {
    ...
    if (gameState.crashState.active) {
        ...
        if (audio.carbackground && !audio.carbackground.paused) audio.carbackground.pause();
    ...
    if (currentView === 'cockpit') {
        // Upewnij się, że dźwięk z innego widoku jest wyłączony
        if (audio.c_engineidle && !audio.c_engineidle.paused) {
            audio.c_engineidle.pause();
        }
        // Uruchom logikę dźwięków dla kokpitu
        handleOverheatedSounds();
        updateRadio(window.deltaTime);
        ...
        // === GŁÓWNA POPRAWKA DŹWIĘKU JEST TUTAJ ===
        // Upewnij się, że dźwięki z kokpitu są wyłączone
        stopAllEngineSounds();
        if (audio.opponent && !audio.opponent.paused) {
            audio.opponent.pause();
        }

        // Teraz sprawdzamy, czy silnik jest przegrzany
        if (gameState.isOverheating) {
            // Silnik jest zepsuty - upewnij się, że wszystkie dźwięki pracy są wyłączone.
            if (audio.c_engineidle && !audio.c_engineidle.paused) audio.c_engineidle.pause();
            if (audio.carbackground && !audio.carbackground.paused) audio.carbackground.pause();
        } else {
            // Silnik działa normalnie - włącz odpowiednie dźwięki tła.
            if (audio.c_engineidle && audio.c_engineidle.paused) {
                audio.c_engineidle.currentTime = 0;
                audio.c_engineidle.volume = 0.6;
                audio.c_engineidle.play().catch(e => console.warn("Nie udało się odtworzyć c_engineidle.mp3", e));
            }
            if (audio.carbackground && audio.carbackground.paused) {
                audio.carbackground.play().catch(e => console.warn("Nie udało się odtworzyć carbackground.mp3", e));
            }
        }
        // === KONIEC POPRAWKI DŹWIĘKU ===
    } else {
...
function updateGameState(deltaTime) {
    ...
    if (pKeyHoldState.isDown && !pKeyHoldState.actionTriggered) {
        pKeyHoldState.timer += deltaTime;
        if (pKeyHoldState.timer >= pKeyHoldState.holdDuration) {
            uruchomSpryskiwacze();
            pKeyHoldState.actionTriggered = true; // Zaznacz, że akcja została wykonana
        }
    }
    if (introAnimationState.isActive) {
    ...
    if (currentView === 'cockpit') {
       ...
       updatePixelationEffect(deltaTime);
        ...
        updateWashingEffect(deltaTime);
        updateSplashEffect(deltaTime); // <-- DODAJ TĘ LINIĘ
        // --- NOWY KOD: Dodaj tę linię ---
        updateMudEffect(deltaTime);
        ...
        if (gameState.oilAmount <= 0 && !gameState.engineSeized) {
         ...
         if (gameState.speedKmH > 0 && !gameState.isOverheating) {
            ...
            const oilLossPerSecond = 4.0 / 120.0; 
        ...
        if (gameState.speedKmH > 84 || gameState.speedKmH < 30) { gameState.temperature += gameState.temperatureIncreaseRate; } else { gameState.temperature -= gameState.temperatureDecreaseRate ; } gameState.temperature = Math.max(gameState.minTemperature, Math.min(gameState.maxTemperature, gameState.temperature)); if ((gameState.temperature >= gameState.maxTemperature || gameState.rpm > 9600) && !gameState.isOverheating) {
    if (gameState.rpm > 9600) {
        console.log("Obroty przekroczyły 9600! Przegrzanie silnika.");
    } else {
        console.log("Silnik przegrzany od temperatury!");
    }
    triggerEngineFailure();
}
        ...
        // --- Logika kolizji z pachołkami (pozostaje bez zmian, jest przed wstrząsami) ---
        const hittableCones = roadsideObjects.trafficCones;
        if (hittableCones.length > 0) {
            const roadWidthTop = canvas.width * 0.001;
            const roadWidthBottom = canvas.width * 1.7;
            const carHitboxWidth = 600;

            for (let i = hittableCones.length - 1; i >= 0; i--) {
                const cone = hittableCones[i];
                const t = 1 - cone.sliceIndex / gameState.roadsideMaxSlices;
                ...
                if (t > 0.96 && t < 1.0) {
                    ...
                    const roadLeftX = (canvas.width - width) / 2 + lateralOffset;
                    const roadRightX = roadLeftX + width;
                    const baseOffsetInsideRoad = 150;
                    const totalOffset = baseOffsetInsideRoad + (cone.lateralOffset || 0);
                    let coneScreenX;
                    if (cone.side === "left") {
                        coneScreenX = roadLeftX + (totalOffset * t);
                    } else {
                        coneScreenX = roadRightX - (totalOffset * t);
                    }

                    const baseSize = width * 0.22;
                    const objSize = baseSize * cone.sizeVariation;
                    const coneHitboxWidth = Math.round(objSize * 2);
                    const collisionDistance = (carHitboxWidth / 2) + (coneHitboxWidth / 2);

                    if (Math.abs((canvas.width / 2) - coneScreenX) < collisionDistance) {
                        if (audio.unlocked && audio.objecthit) {
                            audio.objecthit.currentTime = 0;
                            audio.objecthit.volume = 0.7;
                            audio.objecthit.play().catch(e => console.warn("Nie udało się odtworzyć objecthit.mp3", e));
                        }
                        gameState.coneHitShake.active = true;
                        gameState.coneHitShake.timer = 0;
activeConeHits.push({
    x: canvas.width / 2 + (Math.random() - 0.5) * 150,
    y: canvas.height /2, // Startuje na samym dole ekranu (w miejscu uderzenia)
    size: 380 + Math.random() * 150, // Może być nieco większy
    alpha: 1.0,
    rotation: (Math.random() - 0.5) * 4, // Nadaj mu losowy kąt początkowy
    rotationSpeed: (Math.random() - 0.8) * 35, // Zwiększamy prędkość obrotu
    velocityY: 800 + Math.random() * 650, // KLUCZOWA ZMIANA: Znacznie większa prędkość wylotu w górę
    lifetime: 0,
    maxLifetime: 1.2 + Math.random() * 2 // Lekko wydłużamy czas życia, aby było go widać
});
                        hittableCones.splice(i, 1);
                    }
                }
            }
        }

        // --- POPRAWIONY ŁAŃCUCH WSTRZĄSÓW ---
        if (gameState.crashState.active && gameState.crashState.effectTimer < gameState.crashState.crashShakeDuration) {
            const progress = gameState.crashState.effectTimer / gameState.crashState.crashShakeDuration;
            const currentIntensity = gameState.crashState.crashShakeIntensity * (1 - progress);
            gameState.shakeX += (Math.random() - 0.5) * 2 * currentIntensity;
            gameState.shakeY += (Math.random() - 0.5) * 2 * currentIntensity;
        } else if (gameState.overheatShake.active) {
            gameState.overheatShake.timer += deltaTime;
            if (gameState.overheatShake.timer < gameState.overheatShake.duration) {
                const progress = gameState.overheatShake.timer / gameState.overheatShake.duration;
                const currentIntensity = gameState.overheatShake.intensity * (1 - progress);
                gameState.shakeX += (Math.random() - 0.5) * 2 * currentIntensity;
                gameState.shakeY += (Math.random() - 0.5) * 2 * currentIntensity;
            } else gameState.overheatShake.active = false;
        } else if (gameState.gearShiftShake.active) {
            gameState.gearShiftShake.timer += deltaTime;
            if (gameState.gearShiftShake.timer < gameState.gearShiftShake.duration) {
                const progress = gameState.gearShiftShake.timer / gameState.gearShiftShake.duration;
                const currentIntensity = gameState.gearShiftShake.intensity * (1 - progress);
                gameState.shakeX += (Math.random() - 0.5) * 2 * currentIntensity;
                gameState.shakeY += (Math.random() - 0.5) * 2 * currentIntensity;
            } else gameState.gearShiftShake.active = false;
        } else if (gameState.collisionShake.active) {
            gameState.collisionShake.timer += deltaTime;
            if (gameState.collisionShake.timer < gameState.collisionShake.duration) {
                const progress = gameState.collisionShake.timer / gameState.collisionShake.duration;
                const currentIntensity = gameState.collisionShake.intensity * (1 - progress);
                gameState.shakeX += (Math.random() - 0.5) * 2 * currentIntensity;
                gameState.shakeY += (Math.random() - 0.5) * 2 * currentIntensity;
            } else gameState.collisionShake.active = false;
        } else if (gameState.coneHitShake.active) { // <--- POPRAWNIE WPIĘTY BLOK
            gameState.coneHitShake.timer += deltaTime;
            if (gameState.coneHitShake.timer < gameState.coneHitShake.duration) {
                const progress = gameState.coneHitShake.timer / gameState.coneHitShake.duration;
                const currentIntensity = gameState.coneHitShake.intensity * (1 - progress);
                gameState.shakeX += (Math.random() - 0.5) * 2 * currentIntensity;
                gameState.shakeY += (Math.random() - 0.5) * 2 * currentIntensity;
            } else {
                gameState.coneHitShake.active = false;
            }
        } else { // Zmieniamy ostatni `else if` na `else`, aby normalne drgania były obsługiwane
            const ignitionShake = gameState.ignitionShake;
            const targetIntensity = ignitionState.isTryingToStart ? ignitionShake.maxIntensity : 0.0;
            const easeFactor = ignitionState.isTryingToStart ? ignitionShake.easeInFactor : ignitionShake.easeOutFactor;
            ignitionShake.currentIntensity += (targetIntensity - ignitionShake.currentIntensity) * easeFactor;

            if (ignitionShake.currentIntensity > 0.1) {
                const intensity = ignitionShake.currentIntensity;
...
function drawEngineView() {
    ...
    if (assets.engineBody && assets.engineBody.complete) {
        ...
        // === GŁÓWNA POPRAWKA JEST TUTAJ ===
        // Zmieniamy `engineViewState.shakeOffsetX` na globalne `gameState.shakeX` i `gameState.shakeY`.
        // Dzięki temu silnik będzie reagował na wszystkie globalne wstrząsy,
        // w tym ten z przegrzania (overheatShake), kolizji czy jazdy po trawie.
        // Jeśli silnik pracuje normalnie, `engineViewState.shakeOffsetX` nadal będzie dodawany wewnątrz.
        ctx.translate(
            canvas.width / 2 + gameState.shakeX + engineViewState.shakeOffsetX,
            canvas.height / 2 + gameState.shakeY
        );
        // === KONIEC POPRAWKI ===
    ...
    // Rysujemy korek PO silniku, ale PRZED dymem. Musimy go wywołać wewnątrz, 
    // aby również na niego wpłynęły globalne wstrząsy.
    drawOilCap();
    ...
    if (assets.dust_blue && assets.dust_blue.complete && engineViewState.smokeParticles.length > 0) {
        ...
        for (const p of engineViewState.smokeParticles) {
            ctx.globalAlpha = p.alpha;
            // Dym jest częścią "świata", więc nie dodajemy do niego drgań kamery
            ctx.drawImage(assets.dust_blue, p.x - p.size / 2, p.y - p.size / 2, p.size, p.size);
        }
        ctx.restore();
    }

    // Obszar sprawdzania oleju (pozostaje bez zmian)
    ctx.save();
    ctx.fillStyle = 'rgba(255, 255, 0, 0)'; // Niewidzialny
    ctx.fillRect(oilCheckAreaState.x, oilCheckAreaState.y, oilCheckAreaState.width, oilCheckAreaState.height);
    ctx.restore();
}


const hoverCounterState = {
    isCounterVisible: false, // Czy licznik jest aktualnie widoczny
    mouseX: 0,             // Ostatnia pozycja X myszy
    mouseY: 0,             // Ostatnia pozycja Y myszy
        // --- NOWE WŁAŚCIWOŚCI DO ANIMACJI ---
    alpha: 0.0,           // Aktualna przezroczystość, zaczynamy od niewidocznego
    fadeSpeed: 10.5,       // Szybkość animacji (większa wartość = szybszy fade)
    // --- KONIEC NOWYCH WŁAŚCIWOŚCI ---

    // Definicja samego niebieskiego, interaktywnego obszaru
    interactiveArea: {
        x: 780,
        y: 690,
        width: 70,
...
function updateHoverCounter(deltaTime) {
    const state = hoverCounterState;
    // Określamy docelową przezroczystość: 1.0 (widoczny) lub 0.0 (niewidoczny)
    const targetAlpha = state.isCounterVisible ? 1.0 : 0.0;

    // Jeśli aktualna przezroczystość różni się od docelowej, animujemy ją
    if (Math.abs(state.alpha - targetAlpha) > 0.001) {
        // Płynnie zbliżamy aktualną alfę do docelowej wartości
        // Ta metoda tworzy naturalny efekt "ease-out"
        state.alpha += (targetAlpha - state.alpha) * state.fadeSpeed * deltaTime;
    } else {
        // Jeśli jest już bardzo blisko, "dopinamy" ją do wartości docelowej
        state.alpha = targetAlpha;
    }
}

// --- POPRAWIONA WERSJA FUNKCJI RYSUJĄCEJ KOREK OLEJU ---
// Upewnij się, że ta funkcja również zostanie podmieniona, aby korek poruszał się razem z silnikiem.
function drawOilCap() {
    const state = oilCapState;
    ...
    ctx.save();
    // Przesuwamy punkt obrotu do środka aktualnej pozycji korka
    // +++ DODAJEMY GLOBALNE DRGANIA gameState.shakeX/Y ORAZ LOKALNE engineViewState.shakeOffsetX +++
    ctx.translate(
        state.x + state.width / 2 + gameState.shakeX + engineViewState.shakeOffsetX, 
        state.currentY + state.height / 2 + gameState.shakeY
    );
    // Obracamy korek
    ctx.rotate(state.currentAngle);
    // Rysujemy obrazek, centrując go na punkcie obrotu
...
function drawInteractiveArea() {
    // Rysuj ten obszar tylko i wyłącznie w widoku kokpitu
    if (currentView !== 'cockpit') {
        return;
    }

    const area = hoverCounterState.interactiveArea;
...
updateBlinkEffect(deltaTime); // <--- DODAJ TĘ LINIĘ TUTAJ
    ...
    if (!isGamePaused) {
        ...
        updateHoverCounter(deltaTime);
        updateSpryskiwacze(deltaTime);
        updateMuchy(deltaTime); 
        updateHeldOil(deltaTime);
        updateHeldJug(deltaTime);
        updateHeldWheel(deltaTime);
        updateHeldBeer(deltaTime);
        updateDrinkingAnimation(deltaTime);
        updateThrownBeers(deltaTime);
        updateDipstick(deltaTime); // <--- DODAJ TĘ LINIĘ
        updateHeldCigarette(deltaTime);
                updateSmokingAnimation(deltaTime);
        updateThrownCigarettes(deltaTime);
        updateOilDrops(deltaTime);
        wasWaterOnEngineThisFrame = updateWaterDropsAndCooling(deltaTime); 
        updateSteamParticles(deltaTime); // <-- NOWY KOD
        manageFryingSound(wasWaterOnEngineThisFrame); // <-- NOWY KOD
        managePouringSound();
        updateConeHits(deltaTime); // <--- DODAJ TĘ LINIĘ
        ...
        if (currentView === 'trunk') {
            ...
            updateTrunkJug(deltaTime);
            updateTrunkWheel(deltaTime);
            updateTrunkBeer(deltaTime);
            updateBrakeCap(deltaTime); // <-- DODAJ TĘ LINIĘ
            updateControls(deltaTime);
        }
    }
    ...
    if (currentView === 'cockpit') {
        ...
        drawGround();
        drawSecondGrass();
        drawFences(); // <--- DODAJ TĘ LINIĘ TUTAJ
        drawPickets();
        drawLanterns();
        drawSigns();
        ...
        drawTrafficCones(); // Pachołki rysowane po słupkach
...
drawTurnSignals();
    ...
    drawEngineView(); 
    drawTurnSignals();
    drawDipstick();          // Rysuje bagnet
    drawDipstickOilBar();
        drawSteamParticles(); // <-- NOWY KOD: Rysuj parę na wierzchu silnika
        ...
        drawTrunkOil();
        drawTrunkJug(); // <-- DODAJ TĘ LINIĘ
        drawTrunkWheel();
        drawTrunkBeer();
         drawBrakeCap(); // <-- DODAJ TĘ LINIĘ
        drawTurnSignals(); // <--- ZMIANA TUTAJ
    }
    
    // Rysowanie trzymanych obiektów (są częścią sceny)
    drawHeldOil();
    drawHeldJug(); // <-- DODAJ TĘ LINIĘ
    drawHeldWheel();
    drawHeldBeer();
    drawHeldCigarette();
    drawOilDrops();
    drawWaterDrops(); // <-- DODAJ TĘ LINIĘ
...
if (hoverCounterState.alpha > 0) {
    drawHoverCounter(hoverCounterState.mouseX, hoverCounterState.mouseY);
}
    drawViewTransition();

    // --- SEKCJA 4: RYSOWANIE GLOBALNYCH NAKŁADEK I MENU ---
    // (Te elementy są zawsze na wierzchu i nie podlegają efektom ze sceny)
    drawDrinkingAnimation();
    drawThrownBeers();
        drawThrownCigarettes();
    drawSmokingAnimation();
...
window.addEventListener('load', () => { initCanvas(); createMapButton(); createPhotoButton(); updateButtonPositions(); });
window.addEventListener('resize', updateButtonPositions);
...
if (isMouseOver(mousePos, cigaretteBoxState.rect) && !heldOilState.isHeld && !heldWheelState.isHeld && !heldBeerState.isHeld && !dipstickState.isShown && !heldCigaretteState.isHeld) {
        
        heldCigaretteState.isHeld = true;
        heldCigaretteState.x = mousePos.x;
        heldCigaretteState.y = mousePos.y;
        heldCigaretteState.lastMouseX = mousePos.x;
        heldCigaretteState.angle = 0;
        heldCigaretteState.angleVelocity = 0;

        // Opcjonalnie: odtwórz dźwięk podniesienia
        if (audio.unlocked && audio.press) {
            audio.press.currentTime = 0;
            audio.press.volume = 0.5;
            audio.press.play().catch(() => {});
        }
        return; // Zakończ, aby nie aktywować innych kliknięć
    }

        // Sprawdzamy, czy kursor jest nad stacyjką
        if (isMouseOver(mousePos, ignitionState.rect)) {
            // Warunek 1: Zgaszenie działającego auta (można to zrobić tylko raz)
            if (!gameState.isOverheating && !ignitionState.wasManuallyShutDown) {
                ignitionState.wasManuallyShutDown = true;
                gameState.isOverheating = true; // Używamy istniejącego stanu do zgaszenia i wyciszenia dźwięków

                if (audio.unlocked && audio.turningoff) {
                    audio.turningoff.currentTime = 0;
                    audio.turningoff.play().catch(e => console.warn("Nie udało się odtworzyć turningoff.mp3", e));
                }
                joltKeyrings();
            } 
            // Warunek 2: Próba odpalenia już zgaszonego auta
            else if (gameState.isOverheating) { 
                ignitionState.isTryingToStart = true;


                if (audio.unlocked && audio.ignition && audio.ignition.paused) {
                    audio.ignition.play().catch(e => console.warn("Nie udało się odtworzyć ignition.mp3", e));
                }joltKeyrings();
            }
            return; // Ważne, aby nie aktywować innych klikalnych elementów, jak klakson
        }
    }

    // --- LOGIKA DLA WIDOKU SILNIKA (engine) ---
    if (currentView === 'engine') {
        if (isMouseOver(mousePos, oilCheckAreaState) && !dipstickState.isShown) {
            if (!heldOilState.isHeld && !heldWheelState.isHeld && !heldBeerState.isHeld) {
                dipstickState.isShown = true;
                
                dipstickState.x = mousePos.x;
                dipstickState.y = mousePos.y;
                dipstickState.lastMouseX = mousePos.x;
                dipstickState.angle = 0;
                dipstickState.angleVelocity = 0;

                // <--- DODANO ODTWARZANIE DŹWIĘKU ---
                if (audio.unlocked && audio.dipstick) {
                    audio.dipstick.currentTime = 0;
                    audio.dipstick.volume = 0.7; // Możesz dostosować głośność
                    audio.dipstick.play().catch(e => console.warn("Nie udało się odtworzyć dipstick.mp3", e));
                }
    ...
    if (currentView === 'trunk') {
        ...
        if (isMouseOver(mousePos, trunkJugState) && trunkJugState.isVisible && !heldOilState.isHeld && !heldJugState.isHeld && !heldWheelState.isHeld && !heldBeerState.isHeld) {
            heldJugState.isHeld = true;
            heldJugState.lastMouseX = mousePos.x;
            heldJugState.x = mousePos.x;
            heldJugState.y = mousePos.y;
            trunkJugState.isVisible = false; // Ukryj wersję w bagażniku
            if (audio.unlocked && audio.press) {
                audio.press.currentTime = 0;
                audio.press.volume = 0.5;
                audio.press.play().catch(() => {});
        ...
        // ... (reszta logiki dla bagażnika bez zmian)
    ...
    // ... (reszta listenera mousedown bez zmian)
    if (mapVisible) {
...
if (isMouseOver(mousePos, radioState.rewindButton) && radioState.volume > 0) {
        if (!radioState.isRewinding) {
...
canvas.addEventListener('mousemove', (e) => {
    ...
    // =========================================================================
    // === NOWY BLOK LOGIKI: Sprawdzanie najechania na interaktywny obszar ===
    // =========================================================================
    // Ta logika działa globalnie, ale licznik pojawi się tylko w kokpicie
const area = hoverCounterState.interactiveArea;
    if (currentView === 'cockpit' && isMouseOver(mousePos, area)) {
        // Jesteśmy nad obszarem: POKAŻ licznik i zaktualizuj jego pozycję
        hoverCounterState.isCounterVisible = true;
        hoverCounterState.mouseX = mousePos.x;
        hoverCounterState.mouseY = mousePos.y;
    } else {
        // Jesteśmy POZA obszarem (lub w innym widoku): UKRYJ licznik
        hoverCounterState.isCounterVisible = false;
    }

if (heldJugState.isHeld) {
        const mousePos = getCanvasMousePos(e.clientX, e.clientY);
        const deltaX = mousePos.x - heldJugState.lastMouseX;
        
        heldJugState.x = mousePos.x;
        heldJugState.y = mousePos.y;
        
        const flick = deltaX * heldJugState.flickFactor;
        heldJugState.angleVelocity += flick;
        heldJugState.lastMouseX = mousePos.x;

        // --- POCZĄTEK NOWEGO KODU ---
        // Logika wylewania (skopiowana z oleju, ale tworzy kropelki wody)
        const STREAM_ANGLE_THRESHOLD = 45 * (Math.PI / 180); // Próg 45 stopni
        if (Math.abs(heldJugState.angle) > STREAM_ANGLE_THRESHOLD) {
             const dropsPerFrame = 4; // Ile kropli ma się pojawić w jednej klatce
             for (let i = 0; i < dropsPerFrame; i++) {
                 // Pozycja "dzióbka" dzbanka względem jego środka
                 const spoutOffsetX = (heldJugState.width / 2) - 80;
                 const spoutOffsetY = (-heldJugState.height / 2) + 18;
                 
                 // Obliczanie finalnej pozycji dzióbka z uwzględnieniem obrotu dzbanka
                 const rotatedSpoutX = heldJugState.x + (spoutOffsetX * Math.cos(heldJugState.angle) - spoutOffsetY * Math.sin(heldJugState.angle));
                 const rotatedSpoutY = heldJugState.y + (spoutOffsetX * Math.sin(heldJugState.angle) + spoutOffsetY * Math.cos(heldJugState.angle));
                 
                 // Dodajemy nową kropelkę do globalnej tablicy waterDrops
                 waterDrops.push({ 
                     ...
                     alpha: 1.0, 
                     lifetime: 0, 
                     maxLifetime: 1.5 + Math.random() * 3.0 
                 });
             }
        }
        // --- KONIEC NOWEGO KODU ---
...
if (heldCigaretteState.isHeld) {
        const mousePos = getCanvasMousePos(e.clientX, e.clientY);
        const deltaX = mousePos.x - heldCigaretteState.lastMouseX;
        
        heldCigaretteState.x = mousePos.x;
        heldCigaretteState.y = mousePos.y;
        
        const flick = deltaX * heldCigaretteState.flickFactor;
        heldCigaretteState.angleVelocity += flick;
        heldCigaretteState.lastMouseX = mousePos.x;
    }

    if (dipstickState.isShown) {
        const deltaX = mousePos.x - dipstickState.lastMouseX;
        dipstickState.x = mousePos.x;
        dipstickState.y = mousePos.y;
        
        const flick = deltaX * dipstickState.flickFactor;
        dipstickState.angleVelocity += flick;
        dipstickState.lastMouseX = mousePos.x;
    }
    ...
    if (heldBeerState.isHeld) {
        ...
        const STREAM_ANGLE_THRESHOLD = 40 * (Math.PI / 180);
        if (Math.abs(heldBeerState.angle) > STREAM_ANGLE_THRESHOLD) {
             const dropsPerFrame = 3;
             for (let i = 0; i < dropsPerFrame; i++) {
                 const spoutOffsetX = 0;
                 const spoutOffsetY = (-heldBeerState.height / 2) - 5;
...
const isOverInteractiveArea = isMouseOver(mousePos, hoverCounterState.interactiveArea);
        ...
        // =================================================================
        // === ZMODYFIKOWANY BLOK ZACZYNA SIĘ TUTAJ ===
        // =================================================================
        else if (currentView === 'engine') {
            // Sprawdzamy najechanie na obszar bagnetu ORAZ na sam korek
            const isOverDipstickArea = isMouseOver(mousePos, oilCheckAreaState) && !dipstickState.isShown;
            const isOverOilCap = isMouseOver(mousePos, oilCapState);

            // Zmień kursor, jeśli mysz jest nad którymkolwiek z tych dwóch obszarów
            if (isOverDipstickArea || isOverOilCap) {
                canvas.style.cursor = '';
            } else {
                canvas.style.cursor = '';
            }
        ...
        // =================================================================
        // === ZMODYFIKOWANY BLOK KOŃCZY SIĘ TUTAJ ===
        // =================================================================
        else if (currentView === 'trunk') {
            ...
            const isOverJug = { x: trunkJugState.x, y: trunkJugState.y, width: trunkJugState.width, height: trunkJugState.height };
            ...
            trunkJugState.isHovered = isOverJug;
            if (isOverOil || isOverWheel || isOverBeer || isOverJug) {
                canvas.style.cursor = '';
            } else {
                canvas.style.cursor = '';
            }
    ...
    if (isOverlayDragging) { 
        const deltaX = mousePos.x - lastOverlayDragX; 
        const deltaY = mousePos.y - lastOverlayDragY; 
        if (Math.abs(deltaX) > 10 || Math.abs(deltaY) > 10) { 
            if (audio.unlocked && audio.mapMove.paused) { 
                audio.mapMove.currentTime = 0; 
                audio.mapMove.volume = 0.4; 
                audio.mapMove.play().catch(() => {}); 
            } 
        } 
        if (mapVisible) { 
            mapOffsetX += deltaX; 
            ...
            carSignOffsetX += deltaX; 
            carSignOffsetY += deltaY; 
        } else if (docOverlayVisible) { 
            docOverlayOffsetX += deltaX; 
            docOverlayOffsetY += deltaY; // <--- POPRAWKA JEST TUTAJ
        }
    
        const dragSpeed = Math.sqrt(deltaX * deltaX + deltaY * deltaY); 
        mapSwayVelocity += deltaX * 0.5 * Math.min(dragSpeed / 30, 0.1); 
        mapSwayVelocity = Math.max(-1.1, Math.min(1.1, mapSwayVelocity)); 
        lastOverlayDragX = mousePos.x; 
        lastOverlayDragY = mousePos.y; 
    }
    
    if (hangerState.isDragging) { 
        const dx = mousePos.x - hangerState.anchorX; 
        const dy = mousePos.y - hangerState.anchorY; 
        hangerState.angle = Math.atan2(dx, dy) * -1; 
        const flickSpeed = (mousePos.x - hangerState.lastMouseX) * 0.0015; 
        hangerState.velocity = flickSpeed; 
        hangerState.lastMouseX = mousePos.x; 
    }
});

canvas.addEventListener('mouseup', () => { 
if (ignitionState.isTryingToStart) {
        ignitionState.isTryingToStart = false;

        if (audio.unlocked && audio.ignition && !audio.ignition.paused) {
            audio.ignition.pause();
            audio.ignition.currentTime = 0; // Zresetuj dźwięk na następne wciśnięcie
        }
    }
    // --- KONIEC NOWEGO KODU ---
...
canvas.addEventListener('mouseleave', () => { 
    // --- NOWY KOD: Puszczenie przycisku stacyjki ---
    if (ignitionState.isTryingToStart) {
        ignitionState.isTryingToStart = false;

        if (audio.unlocked && audio.ignition && !audio.ignition.paused) {
            audio.ignition.pause();
            audio.ignition.currentTime = 0;
        }
    }
...
window.addEventListener("keydown", (e) => {
    // --- Obsługa odłożenia obiektów (w tym bagnetu) klawiszem ESC ---
    if (e.key.toLowerCase() === 'escape') {

        if (heldJugState.isHeld) {
            e.preventDefault();
            heldJugState.isHeld = false;
            trunkJugState.isVisible = true; // Pokaż wersję w bagażniku ponownie
            return;
        }
        ...
        // NOWY KOD: odkładanie papierosa
        if (heldCigaretteState.isHeld) {
            e.preventDefault();
            heldCigaretteState.isHeld = false;
            return;
        }

        // --- NOWY KOD: Zamykanie nakładek (mapa, zdjęcie, dokument) ---
        if (mapVisible || photoVisible || docOverlayVisible || carSignVisible) {
            e.preventDefault(); // Zapobiegamy innym akcjom klawisza Escape

            // Ukrywamy wszystkie nakładki
            mapVisible = false;
            photoVisible = false;
            docOverlayVisible = false;
            carSignVisible = false; // Na wszelki wypadek dodajemy też ten

            // Odtwarzamy dźwięk zamknięcia (ten sam co otwarcia dla spójności)
            if (audio.unlocked && audio.map) {
                audio.map.currentTime = 0;
                audio.map.volume = 0.5;
                audio.map.play().catch(() => {});
            }
            return; // Kończymy obsługę, aby nie wykonywać poniższych akcji
        }
        // --- KONIEC NOWEGO KODU ---
    ...
    if (vhsMenuState.isOpen) {
        ...
        const handleMenuAction = (direction) => {
            ...
            const isOptionDisabled = !vhsMenuState.isVcrEffectOn && vhsMenuState.selectedOptionIndex !== 0 && vhsMenuState.selectedOptionIndex !== 3;
    ...
    // ... (reszta listenera keydown bez zmian)
...
case 'p':
    if (!pKeyHoldState.isDown) { // Zapobiega powtarzaniu przy przytrzymaniu
        pKeyHoldState.isDown = true;
        pKeyHoldState.timer = 0;
        pKeyHoldState.actionTriggered = false;
    }
    break; // <-- TUTAJ JEST POPRAWKA

case 'j':
    buttonState.b2.isPressed = !buttonState.b2.isPressed;
    if (audio.unlocked && audio.press) {
        audio.press.currentTime = 0;
        audio.press.volume = 0.7;
        audio.press.play().catch(() => {});
    }
    break;
...
if (e.key.toLowerCase() === 'p') {
        if (pKeyHoldState.isDown) {
            // Jeśli akcja przytrzymania NIE została uruchomiona, to było to krótkie kliknięcie
            if (!pKeyHoldState.actionTriggered) {
                wipersState.active = !wipersState.active;
                if (audio.unlocked && audio.press) {
                    audio.press.currentTime = 0;
                    audio.press.volume = 0.8;
                    audio.press.play().catch(() => {});
            ...
            // Zresetuj stan niezależnie od tego, czy było to krótkie, czy długie naciśnięcie
            pKeyHoldState.isDown = false;
        }
    }
...
canvas.addEventListener('dblclick', (e) => {
    ...
    if (heldCigaretteState.isHeld && !smokingAnimationState.isActive) {
        const mousePos = getCanvasMousePos(e.clientX, e.clientY);
        const cigaretteRect = {
            width: heldCigaretteState.width,
            height: heldCigaretteState.height,
            x: heldCigaretteState.x - heldCigaretteState.width / 2,
            y: heldCigaretteState.y - heldCigaretteState.height / 2,
        };

        if (isMouseOver(mousePos, cigaretteRect)) {
            e.preventDefault();

            // Uruchom animację palenia
            heldCigaretteState.isHeld = false; // Papieros jest zużyty
            smokingAnimationState.isActive = true;
            smokingAnimationState.timer = 0;

            // --- ZMIANA: Odkomentowano i dodano odtwarzanie dźwięku zapalniczki ---
            if (audio.unlocked && audio.lighter) {
                audio.lighter.currentTime = 0; // Zawsze od początku
                audio.lighter.volume = 0.8;    // Możesz dostosować głośność
                audio.lighter.play().catch(e => console.warn("Nie udało się odtworzyć lighter.mp3", e));
            }
            // --- KONIEC ZMIANY ---
            return; // Zakończ, aby nie aktywować logiki picia piwa
        }
    }
    ...
    if (heldBeerState.isHeld && !heldBeerState.isDrinking) {
        ...
        if (isMouseOver(mousePos, beerRect)) {
            ...
            // Rozpoczynamy animację picia (to pozostaje bez zmian)
                heldBeerState.isHeld = false; 
    drinkingAnimationState.isActive = true; 
    
    // --- KLUCZOWA POPRAWKA JEST TUTAJ ---
    drinkingAnimationState.timer = 0;     // Zresetuj główny timer animacji
    drinkingAnimationState.swayTime = 0;  // Zresetuj timer drgania dla spójności
...
canvas.addEventListener('mousedown', (e) => {
    ...
    if (mapVisible) { 
        ...
        const mapY = (canvas.height - mapHeight)/2 + mapOffsetY; 
        if (mousePos.x >= mapX && mousePos.x <= mapX + mapWidth && mousePos.y >= mapY && mousePos.y <= mapY + mapHeight) { 
            isOverlayDragging = true; mapAccordionState.isHeld = true; lastOverlayDragX = mousePos.x; lastOverlayDragY = mousePos.y; canvas.style.cursor = ''; 
            return; 
        } 
    }
    ...
    const docRect = { x: docState.baseX + docState.offsetX, y: docState.baseY + docState.offsetY, width: docState.width * docState.scale, height: docState.height * docState.scale };
    if (isMouseOver(mousePos, docRect) && !mapVisible && !photoVisible && !carSignVisible && !docOverlayVisible) {
        docState.isDragging = true; docState.dragStartMouseX = mousePos.x; docState.dragStartMouseY = mousePos.y; docState.dragStartOffsetX = docState.offsetX; docState.dragStartOffsetY = docState.offsetY; docState.angleVelocity = 0; docState.dragLastMouseX = mousePos.x; canvas.style.cursor = '';
        return;
    }
    ...
    if (photoVisible) { 
        ...
        if (mousePos.x >= photoX && mousePos.x <= photoX + photoWidth && mousePos.y >= photoY && mousePos.y <= photoY + photoHeight) { isOverlayDragging = true; lastOverlayDragX = mousePos.x; lastOverlayDragY = mousePos.y; canvas.style.cursor = ''; return; }
    } else if (carSignVisible) { 
        ...
        if (mousePos.x >= signX && mousePos.x <= signX + signWidth && mousePos.y >= signY && mousePos.y <= signHeight) { isOverlayDragging = true; lastOverlayDragX = mousePos.x; lastOverlayDragY = mousePos.y; canvas.style.cursor = ''; return; }
    } else if (docOverlayVisible) { 
        const currentImage = docOverlayImages.images[docOverlayImages.current];
        if (currentImage && currentImage.complete) {
            ...
            if (mousePos.x >= docX && mousePos.x <= docX + docWidth && mousePos.y >= docY && mousePos.y <= docY + docHeight) { isOverlayDragging = true; lastOverlayDragX = mousePos.x; lastOverlayDragY = mousePos.y; canvas.style.cursor = ''; return; }
        }
    }
    ...
    if (dist < 60) { hangerState.isDragging = true; hangerState.velocity = 0; hangerState.lastMouseX = mousePos.x; canvas.style.cursor = ''; }
});
...
canvas.addEventListener('mousemove', (e) => {
    ...
    if (!isOverlayDragging && !hangerState.isDragging && !docState.isDragging) {
        ...
        // =================================================================
            ...
            // Zmień kursor, jeśli mysz jest nad obszarem sprawdzania oleju I bagnet nie jest wyciągnięty
            if (isMouseOver(mousePos, oilCheckAreaState) && !dipstickState.isShown) {
                canvas.style.cursor = ''; // Ustawia kursor "łapki"
            } else {
    ...
    // Definiujemy prostokąty dla każdego obiektu
    const oilRect = { x: trunkOilState.x, y: trunkOilState.y, width: trunkOilState.width, height: trunkOilState.height };
    const wheelRect = { x: trunkWheelState.x, y: trunkWheelState.y, width: trunkWheelState.width, height: trunkWheelState.height };
    const beerRect = { x: trunkBeerState.x, y: trunkBeerState.y, width: trunkBeerState.width, height: trunkBeerState.height };
    const jugRect = { x: trunkJugState.x, y: trunkJugState.y, width: trunkJugState.width, height: trunkJugState.height }; // Poprawiona definicja

    // Sprawdzamy najechanie dla każdego obiektu
    const isOverOil = isMouseOver(mousePos, oilRect);
    const isOverWheel = isMouseOver(mousePos, wheelRect);
    const isOverBeer = isMouseOver(mousePos, beerRect);
    const isOverJug = isMouseOver(mousePos, jugRect); // Poprawne użycie funkcji

    // Ustawiamy flagi isHovered na podstawie wyników (true/false)
    trunkOilState.isHovered = isOverOil;
    trunkWheelState.isHovered = isOverWheel;
    trunkBeerState.isHovered = isOverBeer;
    trunkJugState.isHovered = isOverJug; // Teraz przypisujemy poprawny wynik

    // Ustawiamy kursor, jeśli jest nad którymkolwiek z obiektów
    if (isOverOil || isOverWheel || isOverBeer || isOverJug) { // Używamy poprawnej zmiennej
        canvas.style.cursor = '';
    } else {
        canvas.style.cursor = '';
    }
